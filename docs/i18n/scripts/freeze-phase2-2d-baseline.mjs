...
const batch = manifest.batches["2D"];
...
for (const relativePath of files) {
  ...
  if (tracked.has(relativePath.replaceAll("\\", "/"))) {
    try {
      const headContent = execFileSync("git", ["show", `HEAD:${relativePath.replaceAll("\\", "/")}`], { cwd: root });
  ...
  entries.push({
    ...
    trackedAtHead: tracked.has(relativePath.replaceAll("\\", "/")),
    headAvailable,
    headSha256,
...
const output = {
  phase: "15.5-stage-2D-settings-models-audio-devices",
  ...
  ownership: {
    ...
    readOnlyIntegration: [
      "frontend/src/components/DisplayLanguageSettings.tsx",
...
const outputPath = path.join(root, "docs/i18n/audit/phase-2-react/2D/pre-edit-baseline.json");
