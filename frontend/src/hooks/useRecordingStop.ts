...
import { completedSnapshotRows } from '@/lib/live-transcript';
...
export function useRecordingStop(
  ...
  const handleRecordingStop = useCallback(async (isCallApi: boolean) => {
    ...
    try {
      ...
      setStatus(RecordingStatus.PROCESSING_TRANSCRIPTS, t('status.waitingForTranscription'));
      ...
      while (elapsedTime < MAX_WAIT_TIME && !transcriptionComplete) {
        try {
          ...
          setStatus(
            ...
            t('status.processingRemainingChunks', { count: status.chunks_in_queue }),
          );
          }
      ...
      if (!transcriptionComplete && elapsedTime >= MAX_WAIT_TIME) {
        ...
        // The native stop path drains the worker queue before reporting completion.
        // Yield briefly so the final event handlers can update React state; a fixed
        // four-second delay made every successful stop unnecessarily slow.
      ...
      setStatus(RecordingStatus.PROCESSING_TRANSCRIPTS, t('status.flushingTranscriptBuffer'));
      flushBuffer();
      ...
      if (isCallApi && transcriptionComplete == true) {
        ...
        setStatus(RecordingStatus.SAVING, t('status.savingMeetingToDatabase'));
        ...
        if (transcriptModelConfig.provider === 'sensevoice') {
          if (!folderPath) throw new Error('Completed recording folder is unavailable');
          freshTranscripts = completedSnapshotRows(await transcriptService.getTranscriptHistory(folderPath));
          const cacheId = sessionStorage.getItem('indexeddb_current_meeting_id');
          if (cacheId) await indexedDBService.saveTranscripts(cacheId, freshTranscripts);
        }
        ...
        try {
          const responseData = await storageService.saveMeeting(
            savedMeetingName || meetingTitle || t('labels.newMeeting'),  // PREFER savedMeetingName (backend source)
            freshTranscripts,
            folderPath
          );
          ...
          const persistedTranscriptCount = freshTranscripts.length;
          const persistedTranscripts = freshTranscripts;
          const transcriptsForLanguageDetection = freshTranscripts.map(transcript => transcript.text);
          // The complete real-time transcript is already durable at this point.
          // Do not automatically decode and transcribe the whole recording again:
          // that duplicated all inference work and locked editing/summary for the
          // duration of the recording. Users can still request the full batch pass
          // explicitly through the existing Enhance action.
          console.info('Recording transcript is ready; full-file enhancement remains user initiated', {
            meeting_id: meetingId,
            provider: transcriptModelConfig.provider,
            transcript_segments: persistedTranscriptCount,
          });
          ...
          try {
            ...
            toast.warning(t('errors.couldNotApplyDefaultSummaryLanguage'), {
              description: t('descriptions.theMeetingWasSavedButTheDefaultSummaryLanguageWas'),
            });
          }
          ...
          if (shouldDetectSummaryLanguage) {
            try {
              await detectAndCacheSummaryLanguage(
                meetingId,
                transcriptsForLanguageDetection
              );
            } catch (error) {
              console.warn('Failed to detect summary language for new meeting:', error);
              toast.warning(t('errors.couldNotDetectSummaryLanguage'), {
                description: t('descriptions.theMeetingWasSavedButAutoCouldNotDetectThe'),
              });
            }
          }
          ...
          try {
            ...
            setCurrentMeeting({ id: meetingId, title: savedMeetingName || meetingTitle || t('labels.newMeeting') });
          }

          // Mark as completed
          setStatus(RecordingStatus.COMPLETED);

          const navigationSource = 'recording';

          // P0-2: 保存结果按"有内容 / 无内容"分支给不同文案，
          // 避免出现"录音已成功保存！已保存 0 个转写片段"这种自相矛盾的提示。
          const viewMeetingAction = {
            label: t('labels.viewMeeting'),
            onClick: () => {
              router.push(`/meeting-details?id=${meetingId}&source=${navigationSource}`);
              Analytics.trackButtonClick('view_meeting_from_toast', 'recording_complete');
            },
          };

          if (persistedTranscriptCount > 0) {
            toast.success(t('messages.recordingSavedSuccessfully'), {
              description: t('messages.transcriptSegmentsSaved', { count: persistedTranscriptCount }),
              action: viewMeetingAction,
              duration: 10000,
            });
          } else {
            toast.warning(t('messages.recordingSavedWithoutSpeech'), {
              description: t('messages.noSpeechDetectedHint'),
              action: viewMeetingAction,
              duration: 12000,
            });
          }

          // The complete real-time transcript is already durably saved, so open it
          // with editing and summary actions available immediately.
          router.push(`/meeting-details?id=${meetingId}&source=${navigationSource}`);
          clearTranscripts();
          ...
          try {
            ...
            if (persistedTranscripts.length > 0 && persistedTranscripts[0].audio_start_time !== undefined) {
              // Use audio_end_time of last transcript if available
              const lastTranscript = persistedTranscripts[persistedTranscripts.length - 1];
              durationSeconds = lastTranscript.audio_end_time || lastTranscript.audio_start_time || 0;
            }

            // Calculate word count
            const transcriptWordCount = persistedTranscripts
            ...
            await Analytics.trackMeetingCompleted(meetingId, {
              ...
              transcript_segments: persistedTranscripts.length,
          ...
          setStatus(RecordingStatus.ERROR, t('errors.failedToSaveMeeting'));
          toast.error(t('errors.failedToSaveMeeting'));
          throw saveError;
        }
      } else {
