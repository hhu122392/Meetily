...
export function useAutoScroll({
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            ...
            // Remember the user's position before new content changes scrollHeight.
    ...
    // Streaming text and virtual row measurements can grow without a new segment.
    useEffect(() => {
        const content = scrollRef.current?.lastElementChild;
        if (!content || !isRecording || isPaused || disableAutoScroll) return;
    ...
    }, [scrollRef, scrollToBottom, isRecording, isPaused, disableAutoScroll]);
    ...
    useEffect(() => {
        ...
        // Follow the previous bottom, not the bottom after the new content has grown.
