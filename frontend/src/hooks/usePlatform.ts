...
export function usePlatform(): Platform {
  ...
  useEffect(() => {
    async function detectPlatform() {
      ...
      // plugin-os exposes its compile-time values through this injected object.
      // Some Meetily builds do not register the native plugin; avoid importing a
      // callable wrapper that would only throw and fall back to the same UA value.
      if (!("__TAURI_OS_PLUGIN_INTERNALS__" in window)) {
        setCurrentPlatform(detectPlatformFromUserAgent());
        return;
      }
