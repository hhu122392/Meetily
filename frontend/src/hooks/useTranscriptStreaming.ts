...
export function useTranscriptStreaming(
  ...
  useEffect(() => {
    ...
    if ((latestSegment.revision ?? 0) > 0 || latestSegment.id === lastSegmentIdRef.current) {
  ...
  }, [latestSegment, isRecording, enableStreaming]);
  ...
  const getDisplayText = (segment: TranscriptSegmentData): string => {
    if ((segment.revision ?? 0) === 0 && streamingSegment && segment.id === streamingSegment.id
      && streamingSegment.fullText === segment.text) {
