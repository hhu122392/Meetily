...
import { isNewerTranscript, mergeLiveTranscripts } from '@/lib/live-transcript';
...
export function TranscriptProvider({ children }: { children: ReactNode }) {
  const { t } = useTranslation(['transcription', 'navigation']);
  const [transcripts, setTranscripts] = useState<Transcript[]>([]);
  const [meetingTitle, setMeetingTitle] = useState(() => t('accessibility.newCall', { ns: 'navigation' }));
  ...
  useEffect(() => {
    if (!currentMeetingId && transcripts.length === 0) {
      setMeetingTitle(t('accessibility.newCall', { ns: 'navigation' }));
    }
  }, [currentMeetingId, transcripts.length, t]);
  ...
  useEffect(() => {
    ...
    const processBufferedTranscripts = (forceFlush = false) => {
      ...
      if (allNewTranscripts.length > 0) {
        setTranscripts(prev => mergeLiveTranscripts(prev, allNewTranscripts));
    ...
    const setupListener = async () => {
      try {
        ...
        unlistenFn = await transcriptService.onTranscriptUpdates((updates) => {
          if (cancelled) return;
          ...
          if (buffered && !isNewerTranscript(buffered, update)) continue;
          ...
          if (currentMeetingId) {
            indexedDBService.saveTranscripts(currentMeetingId, updates)
        ...
        if (cancelled) { unlistenFn(); unlistenFn = undefined; return; }
        console.log('✅ MAIN transcript listener setup complete');
      } catch (error) {
        console.error('❌ Failed to setup MAIN transcript listener:', error);
        alert(t('errors.failedToSetupTranscriptListenerCheckConsoleForDetails', { ns: 'transcription' }));
      }
    };
    ...
    const syncFromBackend = async () => {
      ...
      if (recordingState.isRecording && transcripts.length === 0) {
        try {
          ...
          // A live revision may arrive while history is being fetched.
          setTranscripts(prev => mergeLiveTranscripts(prev, formattedTranscripts));
  ...
  const addTranscript = useCallback((update: TranscriptUpdate) => {
    ...
    setTranscripts(prev => mergeLiveTranscripts(prev, [newTranscript]));
  }, []);
  ...
  const copyTranscript = useCallback(() => {
    ...
    navigator.clipboard.writeText(fullTranscript);
      ...
      toast.success(t('messages.transcriptCopiedToClipboard', { ns: 'transcription' }));
  }, [transcripts, t]);
  ...
  const markMeetingAsSaved = useCallback(async () => {
    ...
    try {
      await indexedDBService.flushTranscriptWrites();
