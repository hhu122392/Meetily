...
export function ConfigProvider({ children }: { children: ReactNode }) {
  ...
  const [selectedDevices, setSelectedDevices] = useState<SelectedDevices>({
    micDevice: null,
    systemDevice: null,
    recordingMode: 'microphone_and_system',
  });
  ...
  useEffect(() => {
    const loadModels = async () => {
      try {
        ...
        if (/timed out|Ollama server/i.test(message)) {
          console.warn('[ConfigContext] Optional Ollama service is unavailable:', err);
        } else {
      ...
      try {
        ...
        if (config) {
          ...
          setTranscriptModelConfig({
            provider: config.provider || DEFAULT_TRANSCRIPT_CONFIG.provider,
            model: config.model || (config.provider === 'parakeet' ? 'parakeet-tdt-0.6b-v3-int8' : config.provider === 'localWhisper' ? 'large-v3-turbo' : DEFAULT_TRANSCRIPT_CONFIG.model),
            apiKey: config.apiKey || null
          });
        }
      } catch (error) {
        if (String(error).includes('state not managed')) {
          console.warn('[ConfigContext] Transcript config is not initialized yet.');
        } else {
  ...
  // Serialize language updates so an async startup/provider change cannot land
  // after a newer explicit user selection.
  const languageSyncQueueRef = useRef<Promise<void>>(Promise.resolve());
  const syncLanguagePreference = useCallback((language: string): Promise<void> => {
    const operation = languageSyncQueueRef.current
      .catch(() => undefined)
      .then(() => invoke<void>('set_language_preference', { language }));
    languageSyncQueueRef.current = operation.catch(() => undefined);
    return operation;
  }, []);

  // Preserve the user's source language. Model compatibility is checked before
  // recording; switching providers must never rewrite this preference.
  useEffect(() => {
    if (!transcriptConfigLoaded) return;

    let cancelled = false;
    const compatibleLanguage = selectedLanguage || 'auto';

    syncLanguagePreference(compatibleLanguage)
      .then(() => {
        if (cancelled) return;
        if (compatibleLanguage !== selectedLanguage) {
          setSelectedLanguage(compatibleLanguage);
          localStorage.setItem('primaryLanguage', compatibleLanguage);
        }
        console.log(
          '[ConfigContext] Synced source language preference to Rust:',
          compatibleLanguage,
        );
      })
      .catch(err => {
  ...
  useEffect(() => {
    const fetchModelConfig = async () => {
      try {
        ...
        if (String(error).includes('state not managed')) {
          console.warn('[ConfigContext] Model config is not initialized yet.');
        } else {
      ...
      try {
        ...
        if (prefs && (prefs.preferred_mic_device || prefs.preferred_system_device)) {
          setSelectedDevices({
            ...
            recordingMode: prefs.recording_mode || 'microphone_and_system',
          });
  ...
  const handleSetSelectedLanguage = useCallback(async (lang: string) => {
    const compatibleLanguage = lang;
    // Wait for the Rust transcription worker to acknowledge the new mode before
    // updating the visible/persisted preference. This prevents a user from
    // selecting Chinese and immediately starting a recording while the backend
    // is still in the previous translate-to-English mode.
    await syncLanguagePreference(compatibleLanguage);
    setSelectedLanguage(compatibleLanguage);
    if (typeof window !== 'undefined') {
      localStorage.setItem('primaryLanguage', compatibleLanguage);
    }
