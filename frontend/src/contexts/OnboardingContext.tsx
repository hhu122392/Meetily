...
import { readSenseVoiceState } from '@/lib/sensevoice';
...
interface OnboardingContextType {
  ...
  currentStep: number;
  sensevoice: ReturnType<typeof useSenseVoiceModel>;
...
export function OnboardingProvider({ children }: { children: React.ReactNode }) {
  ...
  const sensevoice = useSenseVoiceModel();
  ...
  useEffect(() => {
    ...
    if (!isStatusLoaded || completed || isCompletingRef.current) return;
  ...
  }, [currentStep, sensevoice.state.status, summaryModelDownloaded, completed, isStatusLoaded]);
    ...
    try {
      await invoke('save_onboarding_status_cmd', {
        status: {
          ...
          model_status: {
            sensevoice: sensevoice.state.status === 'available' ? 'downloaded' : 'not_downloaded',
  ...
  const saveOnboardingStatusForCompletion = async (summaryReady: boolean, model: string) => {
    const transcription = await readSenseVoiceState();
    if (transcription.status !== 'available') throw new Error('SenseVoice model is not ready');
    await invoke('save_onboarding_status_cmd', { status: {
      version: '1.1', completed: false, current_step: currentStep,
      model_status: { sensevoice: 'downloaded', summary: summaryReady ? 'downloaded' : 'not_downloaded', selected_summary_model: model },
      last_updated: new Date().toISOString(),
    } });
  };
  ...
  const completeOnboarding = async () => {
    try {
      ...
      await saveOnboardingStatusForCompletion(selectedModelReady, modelToSave);
