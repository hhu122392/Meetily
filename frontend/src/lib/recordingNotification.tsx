...
export async function showRecordingNotification(): Promise<void> {
  try {
    const t = i18n.getFixedT(null, 'recording');
    ...
    if (showNotification) {
      ...
      const toastId = toast.info(t('messages.recordingStarted'), {
        description: (
          <div className="space-y-3 min-w-[280px]">
            <p className="text-sm font-medium text-gray-900">
              {t('labels.informAllParticipantsThisMeetingIsBeingRecorded')}
            </p>
            ...
            >
              {t('labels.iVeNotifiedParticipants')}
            </button>
          </div>
        ),
        duration: 10000,
