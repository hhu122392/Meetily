...
type MeetingPageError = 'noMeetingSelected' | 'failedToLoadMeeting';
...
function MeetingDetailsContent() {
  ...
  const [error, setError] = useState<MeetingPageError | null>(null);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [shouldAutoGenerate, setShouldAutoGenerate] = useState<boolean>(false);
  const [hasCheckedAutoGen, setHasCheckedAutoGen] = useState<boolean>(false);
  const [isFinalizingTranscript, setIsFinalizingTranscript] = useState<boolean>(
    source === 'recording-finalizing',
  );
  ...
  useEffect(() => {
    setIsFinalizingTranscript(source === 'recording-finalizing');
  }, [meetingId, source]);

  useEffect(() => {
    if (!meetingId || source !== 'recording-finalizing') return;

    const storageKey = `meetily_recording_finalization_${meetingId}`;
    let disposed = false;
    let settling = false;
    let missingStateChecks = 0;
    let unlistenComplete: UnlistenFn | null = null;
    let unlistenError: UnlistenFn | null = null;

    const settle = async (outcome: 'completed' | 'failed') => {
      ...
      if (!disposed) {
        ...
        router.replace(
          `/meeting-details?id=${meetingId}&source=${outcome === 'completed' ? 'recording' : 'recording-draft'}`,
        );
      }
    };

    const check = async () => {
      const outcome = sessionStorage.getItem(storageKey);
      if (outcome === 'completed' || outcome === 'failed') {
        await settle(outcome);
        return;
      }

      // Recovery for a page reload that loses the JavaScript promise but not the
      // native task: only unlock after the native guard confirms it is idle.
      if (!outcome || outcome === 'pending') {
        const nativeBusy = await invoke<boolean>('is_retranscription_in_progress_command');
        missingStateChecks = nativeBusy ? 0 : missingStateChecks + 1;
        if (missingStateChecks >= 2) await settle('failed');
      }
    };

    const setupNativeListeners = async () => {
      unlistenComplete = await listen<{ meeting_id: string }>(
        'recording-final-transcription-complete',
        (event) => {
          if (event.payload.meeting_id === meetingId) void settle('completed');
        },
      );
      unlistenError = await listen<{ meeting_id: string }>(
        'recording-final-transcription-error',
        (event) => {
          if (event.payload.meeting_id === meetingId) void settle('failed');
        },
      );
    };
  ...
  }, [meetingId, source, refetch, refetchMeetings, router]);
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          <p className="text-red-500 mb-4">{t(`errors.${error}`)}</p>
          <button
            onClick={() => router.push('/')}
