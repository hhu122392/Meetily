...
export function ParakeetModelManager({
  ...
  const localizedModelName = useCallback((modelName: string) => {
    switch (modelName) {
      case 'parakeet-tdt-0.6b-v3-int8': return t('parakeet.lightningName');
      case 'parakeet-tdt-0.6b-v2-int8': return t('parakeet.compactName');
      case 'parakeet-tdt-0.6b-v3-fp32': return t('parakeet.preciseName');
    ...
    const initializeModels = async () => {
      try {
        ...
        toast.error(t('errors.loadTranscriptionModels'), {
          description: t('errors.loadModels'),
          duration: 5000
        });
      } finally {
        setLoading(false);
    ...
    const setupListeners = async () => {
      ...
      unlistenComplete = await listen<{ modelName: string }>(
        ...
        (event) => {
          ...
          const displayName = localizedModelName(modelName);
          ...
          toast.success(`${displayInfo?.icon || '✓'} ${t('messages.downloaded', { model: displayName })}`, {
            description: t('transcription.modelDownloaded'),
            duration: 4000
          });
      ...
      unlistenError = await listen<{ modelName: string; error: string }>(
        ...
        (event) => {
          ...
          toast.error(t('errors.download', { model: displayName }), {
            description: t('errors.download', { model: displayName }),
            duration: 6000,
            action: {
  ...
  const cancelDownload = async (modelName: string) => {
    ...
    try {
      ...
      toast.info(t('messages.downloadCancelled', { model: displayName }), {
        duration: 3000
      });
    } catch (err) {
      console.error('Failed to cancel download:', err);
      toast.error(t('errors.cancelDownload'), {
        description: t('errors.cancelDownload'),
        duration: 4000
      });
    }
  };
  ...
  const downloadModel = async (modelName: string) => {
    ...
    try {
      ...
      toast.info(t('status.downloadingModelEllipsis', { model: displayName }), {
        description: t('transcription.downloadMayTakeTime'),
  ...
  const selectModel = async (modelName: string) => {
    ...
    const displayInfo = getModelDisplayInfo(modelName);
    const displayName = localizedModelName(modelName);
    toast.success(t('messages.switched', { model: displayName }), {
      duration: 3000
    });
  };
  ...
  const deleteModel = async (modelName: string) => {
    ...
    try {
      ...
      toast.success(t('messages.deleted', { model: displayName }), {
        description: t('transcription.modelRemoved'),
        duration: 3000
      });
      ...
      toast.error(t('errors.delete', { model: displayName }), {
        description: t('errors.delete', { model: displayName }),
        duration: 4000
      });
    }
  };
  ...
  if (error) {
    return (
      <div className={`bg-red-50 border border-red-200 rounded-lg p-4 ${className}`}>
        <p className="text-sm text-red-800">{t('transcription.failedToLoad')}</p>
      </div>
    );
  }
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      {selectedModel && (
        ...
        >
          {t('status.usingForTranscription', { model: localizedModelName(selectedModel) })}
        </motion.div>
      )}
    </div>
  );
...
function ModelCard({
  ...
  const displayName = model.name === 'parakeet-tdt-0.6b-v3-int8'
    ? t('parakeet.lightningName')
    : model.name === 'parakeet-tdt-0.6b-v2-int8'
      ? t('parakeet.compactName')
      : model.name === 'parakeet-tdt-0.6b-v3-fp32'
        ? t('parakeet.preciseName')
        : model.name;
  const icon = displayInfo?.icon || '📦';
  const tagline = model.name === 'parakeet-tdt-0.6b-v3-int8'
    ? t('parakeet.lightningTagline')
    : model.name === 'parakeet-tdt-0.6b-v2-int8'
      ? t('parakeet.compactTagline')
      : model.name === 'parakeet-tdt-0.6b-v3-fp32'
        ? t('parakeet.preciseTagline')
        : model.description || '';
