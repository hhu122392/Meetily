...
export type RecordingMode = 'microphone_and_system' | 'microphone_only' | 'system_only';
...
export function DeviceSelection({ selectedDevices, onDeviceChange, disabled = false }: DeviceSelectionProps) {
  ...
  const [error, setError] = useState<
    'devices.loadFailed' | 'devices.noMicrophones' | 'devices.monitoringFailed' | null
  >(null);
  ...
  // A saved selection must stay visible even when the endpoint is currently
  // missing (unplugged headset, driver update, changed endpoint ID). Otherwise
  // the dropdown renders empty and looks as if nothing was ever saved.
  const selectionKey = (device: AudioDevice) =>
    device.native_id ?? `${device.name} (${device.device_type.toLowerCase()})`;
  const micSelectionUnavailable =
    !!selectedDevices.micDevice &&
    !inputDevices.some(device => selectionKey(device) === selectedDevices.micDevice);
  const systemSelectionUnavailable =
    !!selectedDevices.systemDevice &&
    !outputDevices.some(device => selectionKey(device) === selectedDevices.systemDevice);
  ...
  const startAudioLevelMonitoring = async () => {
    try {
      ...
      if (deviceNames.length === 0) {
        setError('devices.noMicrophones');
        return;
      }
      ...
      setError('devices.monitoringFailed');
    }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        <div className="space-y-2">
          <Label htmlFor="recording-mode" className="text-sm font-medium text-gray-700">
            {t('devices.recordingMode')}
          </Label>
          <Select
            value={selectedDevices.recordingMode}
            onValueChange={(recordingMode: RecordingMode) => onDeviceChange({
              ...selectedDevices,
              recordingMode,
            })}
            disabled={disabled}
          >
            ...
            <SelectContent>
              <SelectItem value="microphone_and_system">{t('devices.modeMicrophoneAndSystem')}</SelectItem>
              <SelectItem value="microphone_only">{t('devices.modeMicrophoneOnly')}</SelectItem>
              <SelectItem value="system_only">{t('devices.modeSystemOnly')}</SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            disabled={disabled || selectedDevices.recordingMode === 'system_only'}
          >
            ...
            {micSelectionUnavailable && (
              <SelectItem value={selectedDevices.micDevice as string} disabled>
                {t('devices.unavailableSelection')}
              </SelectItem>
            )}
              {inputDevices.map((device) => (
                <SelectItem
                  key={device.native_id ?? `${device.name}-${device.device_type}`}
                  value={device.native_id ?? `${device.name} (${device.device_type.toLowerCase()})`}
                >
                  {device.name}
                </SelectItem>
              ))}
          ...
          {micSelectionUnavailable && (
            <p className="text-xs text-amber-600">{t('devices.unavailableHint')}</p>
          )}
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            disabled={disabled || selectedDevices.recordingMode === 'microphone_only'}
          >
            ...
            {systemSelectionUnavailable && (
              <SelectItem value={selectedDevices.systemDevice as string} disabled>
                {t('devices.unavailableSelection')}
              </SelectItem>
            )}
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• <strong>{t('devices.microphone')}:</strong> {t('devices.microphoneDescription')}</p>
        <p>• <strong>{t('devices.systemAudio')}:</strong> {t('devices.systemAudioDescription')}</p>
        {isMonitoring && (
          <p>• <strong>{t('devices.microphoneLevels')}</strong> {t('devices.levelsDescription')}</p>
        )}
        {!isMonitoring && inputDevices.length > 0 && (
          <p>• <strong>{t('devices.tip')}</strong> {t('devices.testTip')}</p>
        )}
      </div>
    </div>
  );
