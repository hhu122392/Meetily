...
import { LANGUAGE_OPTIONS, localizedLabelForCode } from "@/lib/summary-languages";
...
export function LanguagePickerPopover({
  ...
  const localizedOptions = useMemo(
    () => LANGUAGE_OPTIONS.map((option) => ({
      ...option,
      label: localizedLabelForCode(option.code, i18n.resolvedLanguage || i18n.language),
    })),
  ...
  const filteredAll = useMemo(() => {
    const options = mode === "meeting"
      ? localizedOptions.filter((l) => !recentCodes.has(l.code))
      : localizedOptions;
  ...
  }, [filter, localizedOptions, mode, recentCodes]);

  const recentsResolved = useMemo(
    () =>
      recents
        .map((code) => localizedOptions.find((l) => l.code === code))
        .filter((l): l is (typeof localizedOptions)[number] => Boolean(l))
        .filter(
          (l) =>
            !filter ||
            l.code.toLowerCase().includes(filter) ||
    ...
    [recents, filter, localizedOptions],
  );
  ...
  return (
    <div
      ...
      role="dialog"
      aria-label={t('languagePicker.pickSummaryLanguage')}
    >
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-gray-100">
        ...
        <input
          ...
          aria-label={t('languagePicker.search')}
      ...
      <div className="max-h-80 overflow-y-auto py-1">
        ...
        {filteredAll.length > 0 && (
          <div className="px-3 pt-1 pb-1 text-[10px] font-semibold uppercase tracking-wider text-gray-400">
            {mode === "meeting" ? t('languagePicker.otherLanguages') : t('languagePicker.allLanguages')}
          </div>
        )}
        ...
        {hasNoResults && (
          <div className="px-3 py-2 text-sm text-gray-400">{t('languagePicker.noMatches')}</div>
        )}
      </div>
    </div>
  );
