...
export function SummaryLanguageSettings() {
  ...
  // P2-21：语言 chip 要按界面语言显示（原来固定用英文名）
  const uiLocale = i18n.resolvedLanguage || i18n.language;
  const languageLabel = (code: string) => localizedLabelForCode(code, uiLocale);
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm relative">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="text-lg font-semibold text-gray-900">{t('summary.languageTitle')}</h3>
      </div>
      <p className="text-sm text-gray-600 mb-4">
        {t('summary.languageDescription')}
      </p>
          ...
          return (
            ...
            >
              <button
                type="button"
                aria-label={isPinned
                  ? t('summary.unpinLanguage', { language: languageLabel(code) })
                  : t('summary.pinLanguage', { language: languageLabel(code) })}
                aria-pressed={isPinned}
                title={isPinned ? t('summary.unsetDefault') : t('summary.setDefault')}
              ...
              <button
                type="button"
                aria-label={t('summary.removeLanguage', { language: languageLabel(code) })}
      ...
      <p className="text-xs text-gray-400 mt-3">
        {pinned
          ? t('summary.languageDefaultHint', { language: languageLabel(pinned) })
          : t('summary.languageHint')}
      </p>
    </div>
  );
}
