...
export function AudioBackendSelector({
  ...
  const [error, setError] = useState<
    'audioBackend.loadFailed' | 'audioBackend.changeFailed' | null
  >(null);
  const [showTooltip, setShowTooltip] = useState(false);
  const backendDescription = (backend: BackendInfo) => {
    if (backend.id === 'screencapturekit') return t('audioBackend.screenCaptureKitDescription');
    if (backend.id === 'coreaudio' || backend.id === 'core_audio') return t('audioBackend.coreAudioDescription');
    return t('audioBackend.defaultDescription');
  };
  ...
  useEffect(() => {
    const loadBackends = async () => {
      try {
        ...
        setError('audioBackend.loadFailed');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleBackendChange = async (backendId: string) => {
    try {
      ...
      setError('audioBackend.changeFailed');
    }
  };
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        ...
        <div className="relative">
          <button
            ...
            aria-label={t('audioBackend.methods')}
            ...
            <div className="absolute z-10 left-6 top-0 w-64 p-3 text-xs bg-gray-900 text-white rounded-lg shadow-lg">
              ...
              <ul className="space-y-1">
                {backends.map((backend) => (
                  <li key={backend.id}>
                    <span className="font-medium">{backend.name}:</span> {backendDescription(backend)}
                  </li>
                ))}
              </ul>
              <p className="mt-2 text-gray-300">
                {t('audioBackend.description')}
              </p>
            </div>
          )}
        </div>
          ...
          return (
            ...
            >
              ...
              <div className="ml-3 flex-1">
                ...
                <p className="mt-1 text-xs text-gray-600">{backendDescription(backend)}</p>
              </div>
            </label>
          );
        })}
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• {t('audioBackend.selectionOnlySystem')}</p>
