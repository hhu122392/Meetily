    ...
    <Dialog aria-describedby={undefined}>
      <DialogTrigger asChild>
        <button 
          ref={ref} 
          type="button"
          aria-label={t('labels.aboutMeetily')}
          ...
          title={t('labels.aboutMeetily')}
        >
          <InfoIcon className={`text-gray-600 ${isCollapsed ? "w-5 h-5" : "w-4 h-4"}`} />
          {!isCollapsed && (
            <span className="ml-2 text-sm text-gray-700">{t('labels.about')}</span>
          )}
        </button>
      </DialogTrigger>
      <DialogContent>
        <VisuallyHidden>
          <DialogTitle>{t('labels.aboutMeetily')}</DialogTitle>
        </VisuallyHidden>
        <About />
      </DialogContent>
    </Dialog>
