...
interface RecordingControlsProps {
  ...
  selectedDevices?: {
    ...
    recordingMode: 'microphone_and_system' | 'microphone_only' | 'system_only';
  };
  meetingName?: string;
}
...
export const RecordingControls: React.FC<RecordingControlsProps> = ({
  ...
  useEffect(() => {
    const checkTauri = async () => {
      try {
        ...
        alert(t('errors.failedToInitializeRecordingPleaseCheckTheConsoleForDetails'));
      }
    };
    checkTauri();
  }, []);
  ...
  const handleStartRecording = useCallback(async () => {
    ...
    // Lock the visible start control immediately. Model/device initialization
    // takes a few seconds, and leaving the button enabled allowed a second
    // click to race the first request and surface a false "recording failed"
    // alert even though recording had already started successfully.
    setIsStarting(true);
    setDeviceError(null);
    ...
    try {
      ...
      // A successful retry must also dismiss an error left by an earlier
      // failed attempt instead of showing success and failure together.
      setDeviceError(null);
    } catch (error) {
      ...
      if (errorMsg.includes('microphone') || errorMsg.includes('mic') || errorMsg.includes('input')) {
        setDeviceError({
          title: t('accessibility.microphoneNotAvailable'),
          message: t('errors.unableToAccessYourMicrophonePleaseCheckThatYourMicrophone')
        });
      } else if (errorMsg.includes('system audio') || errorMsg.includes('speaker') || errorMsg.includes('output')) {
        setDeviceError({
          title: t('accessibility.systemAudioNotAvailable'),
          message: t('errors.unableToCaptureSystemAudioPleaseCheckThatAVirtual'),
          action: 'deviceSettings'
        });
      } else if (errorMsg.includes('permission')) {
        setDeviceError({
          title: t('accessibility.permissionRequired'),
          message: t('status.recordingPermissionsAreRequiredPleaseGrantMicrophoneAccessInSystem')
        });
      } else {
        setDeviceError({
          title: t('status.recordingFailed'),
          message: t('errors.unableToStartRecordingPleaseCheckYourAudioDeviceSettings')
        });
      }
    } finally {
      setIsStarting(false);
  ...
  const handlePauseRecording = useCallback(async () => {
    ...
    try {
      ...
      alert(t('errors.failedToPauseRecordingPleaseCheckTheConsoleForDetails'));
    } finally {
      setIsPausing(false);
    }
  ...
  const handleResumeRecording = useCallback(async () => {
    ...
    try {
      ...
      alert(t('errors.failedToResumeRecordingPleaseCheckTheConsoleForDetails'));
    } finally {
      setIsResuming(false);
    }
      ...
      try {
        ...
        const transcriptErrorUnsubscribe = await listen('transcript-error', (event) => {
          ...
          if (onTranscriptionError) {
              onTranscriptionError(t('errors.transcriptionFailed'));
          }
        });
      ...
      <div className="flex flex-col space-y-2">
        <div className="flex items-center space-x-2 bg-white rounded-full shadow-lg px-4 py-2">
          {isProcessing && !isParentProcessing ? (
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-gray-600">{t('status.processingRecording')}</span>
            </div>
          ) : (
            <>
              {showPlayback ? (
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          ...
                          aria-label={t('actions.startRecording')}
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <button
                            ...
                            aria-label={isPaused ? t('actions.resumeRecording') : t('actions.pauseRecording')}
                          ...
                          >
                            ...
                            {(isPausing || isResuming) && (
                              <div className="absolute -top-8 text-gray-600 font-medium text-xs">
                                {isPausing ? t('status.pausing') : t('status.resuming')}
                              </div>
                            )}
                          </button>
                        </TooltipTrigger>
                        <TooltipContent>
                          <p>{isPaused ? t('actions.resumeRecording') : t('actions.pauseRecording')}</p>
                        </TooltipContent>
                      </Tooltip>
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <button
                            ...
                            aria-label={t('actions.stopRecording')}
        ...
        {isValidatingModel && (
          <div className="text-xs text-gray-600 text-center mt-2">
            {t('status.validatingSpeechRecognition')}
          </div>
        )}
        ...
        {deviceError && (
          <Alert variant="destructive" className="mt-4 border-red-300 bg-red-50">
            ...
            <AlertDescription className="text-red-700">
              ...
              {deviceError.action === 'deviceSettings' && onOpenDeviceSettings && (
                <button
                  type="button"
                  onClick={() => {
                    setDeviceError(null);
