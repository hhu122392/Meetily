...
export function ModelSettingsModal({
  ...
  const fetchOllamaModels = async (silent = false) => {
    ...
    if (trimmedEndpoint && !validateOllamaEndpoint(trimmedEndpoint)) {
      const errorMsg = 'invalid-ollama-endpoint';
      setError(errorMsg);
      if (!silent) {
        toast.error(t('errors.invalidOllamaEndpoint'));
      }
      return;
    }
  ...
  const loadBuiltinAiModels = async () => {
    ...
    try {
      ...
      toast.error(t('errors.loadBuiltInModels'));
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (modelConfig.provider === 'custom-openai') {
      try {
        ...
        toast.error(t('errors.saveCustomConfig'));
        return;
      }
    }
  ...
  const testCustomOpenAIConnection = async () => {
    if (!customOpenAIEndpoint.trim() || !customOpenAIModel.trim()) {
      toast.error(t('errors.endpointAndModelRequired'));
      return;
    }
    ...
    try {
      ...
      console.error('Custom OpenAI connection test failed:', err);
  ...
  const downloadRecommendedModel = async () => {
    ...
    if (isDownloading(recommendedModel)) {
      toast.info(t('messages.alreadyDownloading', { model: recommendedModel }), {
        description: t('status.progressLabel', { progress: Math.round(getProgress(recommendedModel) || 0) })
      });
      return;
    }
    ...
    try {
      ...
      if (isOllamaNotInstalledError(errorMsg)) {
        toast.error(t('errors.ollamaNotInstalled'), {
          description: t('errors.ollamaInstallDescription'),
          duration: 7000,
          action: {
  ...
  const deleteOllamaModel = async (modelName: string) => {
    try {
      ...
      toast.success(t('messages.deleted', { model: modelName }));
      await fetchOllamaModels(true); // Refresh list
    } catch (err) {
      toast.error(t('errors.delete', { model: modelName }));
  ...
  const recommendedOllamaModel = 'gemma3:1b';
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        <div>
          <Label>{t('config.summaryModel')}</Label>
          <div className="flex space-x-2 mt-1">
            ...
            >
              <SelectTrigger>
                <SelectValue placeholder={t('config.selectProvider')} />
              </SelectTrigger>
              <SelectContent className="max-h-64 overflow-y-auto">
                <SelectItem value="builtin-ai">{t('providers.builtIn')}</SelectItem>
                <SelectItem value="claude">{t('providers.claude')}</SelectItem>
                <SelectItem value="custom-openai">{t('providers.customOpenAI')}</SelectItem>
                ...
                <SelectItem value="openrouter">{t('providers.openRouter')}</SelectItem>
              </SelectContent>
            </Select>
            ...
            {modelConfig.provider !== 'builtin-ai' && modelConfig.provider !== 'custom-openai' && (
              <Popover open={modelComboboxOpen} onOpenChange={setModelComboboxOpen} modal={true}>
                <PopoverTrigger asChild>
                  ...
                  >
                    <span className="truncate">
                      {modelConfig.model || t('config.selectModel')}
                    </span>
                ...
                <PopoverContent className="w-[250px] p-0" align="start">
                  <Command>
                    <CommandInput placeholder={t('config.searchModels')} />
                      ...
                      {(modelConfig.provider === 'openrouter' && isLoadingOpenRouter) ||
                       ...
                       (modelConfig.provider === 'groq' && isLoadingGroq) ? (
                        <div className="py-6 text-center text-sm text-muted-foreground">
                          ...
                          {t('status.loadingModels')}
                        </div>
                      ) : (
                        <>
                          <CommandEmpty>{t('config.noModels')}</CommandEmpty>
                          <CommandGroup>
        ...
        {modelConfig.provider === 'custom-openai' && (
          <div className="space-y-4 border-t pt-4">
            <div>
              <div className="flex items-center gap-1"><Label htmlFor="custom-endpoint">{t('config.endpointUrl')}</Label><HelpHint text={t('config.endpointDescription')} /></div>
              <Input
                id="custom-endpoint"
            ...
            <div>
              <div className="flex items-center gap-1"><Label htmlFor="custom-model">{t('config.modelName')}</Label><HelpHint text={t('config.modelDescription')} /></div>
              <Input
                id="custom-model"
            ...
            <div>
              <Label htmlFor="custom-api-key">{t('config.apiKeyOptional')}</Label>
              <Input
                id="custom-api-key"
                type="password"
                value={customOpenAIApiKey}
                onChange={(e) => setCustomOpenAIApiKey(e.target.value)}
                placeholder={t('config.optionalKeyPlaceholder')}
                className="mt-1"
              />
            </div>
            ...
            <div>
              ...
              >
                <Label className="cursor-pointer">{t('config.advanced')}</Label>
              ...
              {isCustomOpenAIAdvancedOpen && (
                <div className="space-y-3 pl-2 border-l-2 border-muted mt-2">
                  <div>
                    <Label htmlFor="custom-max-tokens">{t('config.maxTokens')}</Label>
                    <Input
                  ...
                  <div>
                    <Label htmlFor="custom-temperature">{t('config.temperature')}</Label>
                    <Input
                  ...
                  <div>
                    <Label htmlFor="custom-top-p">{t('config.topP')}</Label>
                    <Input
                      id="custom-top-p"
                      type="number"
                      step="0.1"
                      ...
                      placeholder={t('placeholders.topP')}
                      className="mt-1"
                    />
                  </div>
                </div>
              ...
              <div className="absolute inset-y-0 right-0 pr-1 flex items-center space-x-1">
                {apiKey?.trim() && (
                  <Button
                    ...
                    title={isApiKeyLocked ? t('actions.unlock') : t('actions.lock')}
                  >
        ...
        {modelConfig.provider === 'ollama' && (
          <div>
            ...
            >
              <Label className="cursor-pointer">{t('config.customEndpoint')}</Label>
            ...
            {!isEndpointSectionCollapsed && (
              <>
                <p className="text-sm text-muted-foreground mt-1 mb-2">
                  {t('config.customEndpointDescription')}
                </p>
                ...
                {ollamaEndpointChanged && !error && (
                  <Alert className="mt-3 border-yellow-500 bg-yellow-50">
                    <AlertDescription className="text-yellow-800">
                      {t('config.endpointChangedBeforeSave')}
                    </AlertDescription>
                  </Alert>
                )}
              </>
        ...
        {modelConfig.provider === 'ollama' && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-sm font-bold">{t('config.availableOllama')}</h4>
              {lastFetchedEndpoint && models.length > 0 && (
                <div className="flex items-center gap-2 text-sm">
                  <span className="text-muted-foreground">{t('config.using')}</span>
            ...
            {isLoadingOllama ? (
                ...
                {ollamaNotInstalled ? (
                  ...
                  <div className="space-y-4">
                    ...
                    >
                      ...
                      {t('actions.downloadOllama')}
                    </Button>
                    <div className="text-sm text-muted-foreground text-center">
                      {t('config.ollamaAfterInstall')}
                    </div>
                  </div>
                ) : (
                    ...
                    <Alert className="mb-4">
                      <AlertDescription>
                        {ollamaEndpointChanged
                          ? t('config.endpointChanged')
                          : t('config.noOllamaModels')}
                      </AlertDescription>
                    </Alert>
                    {!ollamaEndpointChanged && (
                      <div className="space-y-3">
                        <Button
                          ...
                          disabled={isDownloading(recommendedOllamaModel)}
                          className="w-full"
                        >
                          {isDownloading(recommendedOllamaModel) ? (
                            <>
                              <RefreshCw className="mr-2 h-4 w-4 animate-spin" />
                              {t('status.downloadingModelEllipsis', { model: recommendedOllamaModel })}
                            </>
                          ) : (
                            <>
                              <Download className="mr-2 h-4 w-4" />
                              {t('actions.downloadRecommended')}
                            </>
                          )}
                        </Button>

                        {/* Show progress for gemma3:1b download */}
                        {isDownloading(recommendedOllamaModel) && getProgress(recommendedOllamaModel) !== undefined && (
                          <div className="bg-white rounded-md border p-3">
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-sm font-medium text-blue-600">{t('status.downloadingModelEllipsis', { model: recommendedOllamaModel })}</span>
                              <span className="text-sm font-semibold text-blue-600">
                                {Math.round(getProgress(recommendedOllamaModel)!)}%
                              </span>
                            </div>
                            <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                              <div
                                className="h-full bg-gradient-to-r from-blue-500 to-blue-600 rounded-full transition-all duration-300"
                                style={{ width: `${getProgress(recommendedOllamaModel)}%` }}
                              />
                            </div>
                          </div>
                        )}
              ...
              <ScrollArea className="max-h-[calc(100vh-450px)] overflow-y-auto pr-4">
                {filteredModels.length === 0 ? (
                  <Alert>
                    <AlertDescription>
                      {t('config.noMatchingModels', { query: searchQuery })}
                    </AlertDescription>
                  </Alert>
                ) : (
                        ...
                        >
                          <div>
                            <span className="text-muted-foreground">
                              {t('download.sizeDescription', { name: model.name, size: model.size })}
                            </span>
                          </div>
