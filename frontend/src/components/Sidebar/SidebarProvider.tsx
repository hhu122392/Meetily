...
import { queueAutoStartRecordingRequest } from '@/lib/recording-auto-start';
...
export function SidebarProvider({ children }: { children: React.ReactNode }) {
  ...
  const [currentMeeting, setCurrentMeeting] = useState<CurrentMeeting | null>({ id: 'intro-call', title: t('accessibility.newCall') });
  // P2-17：默认展开侧边栏，首次打开就能看到会议列表和各入口的文字标签
    ...
    if (serverAddress) {
      try {
        ...
        if (String(error).includes('state not managed')) {
          console.warn('[SidebarProvider] Meetings are not initialized yet.');
        } else {
    ...
    if (!isRecording) {
      ...
      if (pathname === '/') {
        ...
        requestAppNavigation('/', () => {
          void invoke<string>('get_app_session_id')
            .then((appSessionId) => {
              queueAutoStartRecordingRequest(sessionStorage, appSessionId);
              router.push('/');
            })
            .catch((error) => {
              // Fail closed: navigate home, but never record without a native
              // process-bound authorization token.
              sessionStorage.removeItem('autoStartRecording');
              console.error('Failed to authorize recording navigation:', error);
              router.push('/');
            });
        });
      }
  ...
  // Native tray actions are routed through the same process-bound path as the
  // visible sidebar button. This avoids hard page reloads and legacy storage
  // flags that can survive an application restart.
  useEffect(() => {
    const handleNativeRecordingRequest = () => handleRecordingToggle();
    window.addEventListener('request-recording-from-tray', handleNativeRecordingRequest);
    return () => {
      window.removeEventListener('request-recording-from-tray', handleNativeRecordingRequest);
    };
