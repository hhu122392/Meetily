...
interface SidebarItem {
  ...
  /** P1-9：列表里的可分辨信息（由 SidebarProvider 从 api_get_meetings 带入） */
  createdAt?: string;
...
const Sidebar: React.FC = () => {
  ...
  const handleDelete = async (itemId: string) => {
    ...
    try {
      ...
      // Show success toast
      toast.success(t('messages.meetingDeletedSuccessfully'), {
        description: t('descriptions.allAssociatedDataHasBeenRemoved')
      });

      // If deleting the active meeting, navigate to home
      if (currentMeeting?.id === itemId) {
        setCurrentMeeting({ id: 'intro-call', title: t('accessibility.newCall') });
        requestAppNavigation('/', () => router.push('/'));
      }
    } catch (error) {
      console.error('Failed to delete meeting:', error);
      toast.error(t('errors.failedToDeleteMeeting'));
    }
  };
  ...
  const handleEditConfirm = async () => {
    ...
    if (!newTitle) {
      toast.error(t('errors.meetingTitleCannotBeEmpty'));
      return;
    }
    ...
    try {
      ...
      // P2-18：详情页标题也要跟着变，不能等切走再切回
      if (typeof window !== 'undefined') {
        window.dispatchEvent(
          new CustomEvent('meeting-title-updated', {
            detail: { meetingId, title: newTitle },
          }),
        );
      }
      ...
      toast.success(t('messages.meetingTitleUpdatedSuccessfully'));
      ...
      toast.error(t('errors.failedToUpdateMeetingTitle'));
    }
  };
        ...
        <div className="flex flex-col items-center space-y-4 mt-4">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                onClick={() => requestAppNavigation('/', () => router.push('/'))}
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                aria-label={isRecording ? t('status.recordingInProgress') : t('actions.startRecording')}
            ...
            <TooltipContent side="right">
              <p>{isRecording ? t('status.recordingInProgress') : t('actions.startRecording')}</p>
            </TooltipContent>
          </Tooltip>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
                <button
                  ...
                  aria-label={t('labels.meetingNotes')}
  ...
  // P1-9：列表第二行显示"时间 · 时长 · 片段数"，让同名的「会议 11_09_26_xx_xx_xx」能区分开
  const formatMeetingMeta = (item: {
    createdAt?: string;
    durationSeconds?: number;
    transcriptCount?: number;
  }) => {
    ...
    if (seconds > 0) {
      ...
      parts.push(`${minutes}:${String(rest).padStart(2, '0')}`);
    }
    if (typeof item.transcriptCount === 'number') {
      parts.push(t('labels.transcriptCountValue', { count: item.transcriptCount }));
    }
    return parts.join(' · ');
  };
          ...
          {item.type === 'folder' ? (
            <>
              ...
              {searchQuery && item.id === 'meetings' && isSearching && (
                <span className="ml-2 text-xs text-blue-500 animate-pulse">{t('status.searching')}</span>
              )}
            </>
          ) : (
            <div className="flex flex-col w-full">
                ...
                {isMeetingItem && (
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                    <button
                      ...
                      aria-label={t('accessibility.editMeetingTitle')}
                    >
                    ...
                    <button
                      ...
                      aria-label={t('accessibility.deleteMeeting')}
                    >
              ...
              {/* P1-9：会议时间 / 时长 / 片段数 */}
              {isMeetingItem && formatMeetingMeta(item) && (
                <div className="ml-8 text-xs text-gray-500">{formatMeetingMeta(item)}</div>
              )}

              {/* Show transcript match snippet if available */}
              {hasTranscriptMatch && (
                <div className="mt-1 ml-8 text-xs text-gray-500 bg-yellow-50 p-1.5 rounded border border-yellow-100 line-clamp-2">
                  <span className="font-medium text-yellow-600">{t('labels.match')}</span> {matchingResult.matchContext}
                </div>
              )}
            </div>
          )}
  ...
  return (
    <div className="fixed top-0 left-0 h-screen z-40">
      ...
      <button
        ...
        aria-label={isCollapsed ? t('accessibility.expandSidebar') : t('accessibility.collapseSidebar')}
            ...
            {!isCollapsed && (
              <div className="p-3">
                ...
                <div className="relative mb-1">
                  <InputGroup >
                    <InputGroupInput placeholder={t('placeholders.searchMeetingContent')} value={searchQuery}
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            {!isCollapsed && (
              <div className="flex-1 overflow-y-auto custom-scrollbar min-h-0">
                ...
                {/* P1-10：搜索没有命中时给明确空态，不再是一片空白 */}
                {searchQuery.trim() &&
                  !isSearching &&
                  !filteredSidebarItems.some(
                    item => item.type === 'folder' && (item.children?.length ?? 0) > 0
                  ) && (
                    <p className="px-6 py-4 text-sm text-gray-500">
                      {t('status.noSearchMatches')}
                    </p>
                  )}
              </div>
            )}
          ...
          <div className="flex-shrink-0 p-2 border-t border-gray-100">
            ...
            >
              {isRecording ? (
                <>
                  ...
                  <span>{t('status.recordingInProgress')}</span>
                </>
              ) : (
                <>
            ...
            {betaFeatures.importAndRetranscribe && (
              ...
              >
                ...
                <span>{t('actions.importAudio')}</span>
              </button>
            )}
      ...
      <ConfirmationModal
        ...
        text={t('descriptions.confirmDeleteMeeting')}
      ...
      <Dialog open={editModalState.isOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-[425px]">
          <VisuallyHidden>
            <DialogTitle>{t('actions.editMeetingTitle')}</DialogTitle>
          </VisuallyHidden>
          <div className="py-4">
            <h3 className="text-lg font-semibold mb-4">{t('actions.editMeetingTitle')}</h3>
            <div className="space-y-4">
              <div>
                <label htmlFor="meeting-title" className="block text-sm font-medium text-gray-700 mb-2">
                  {t('labels.meetingTitle')}
                </label>
                <input
                  id="meeting-title"
                  type="text"
                  ...
                  placeholder={t('placeholders.enterMeetingTitle')}
                  autoFocus
                />
              </div>
            </div>
