    ...
    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm font-medium text-blue-900">
            {isCompleted ? t('status.finalizing') : t('status.downloadingModelEllipsis', { model: modelName })}
          </span>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <div className="flex justify-between text-xs text-blue-700 mt-1">
          <span>{t('status.progress', { progress: Math.round(progress) })}</span>
          {!isCompleted && (
            <span className="animate-pulse">{t('status.downloading')}</span>
          )}
        </div>
      </div>
    ...
    <div className="bg-gray-50 rounded-lg p-3 text-sm">
      <div className="flex items-center justify-between">
        <span className="text-gray-700">
          📦 {t('download.modelsAvailable', { downloaded: downloadedModels, total: totalModels })}
        </span>
        <span className="text-gray-600">
          💾 {t('download.total', { size: formatSize(totalSizeMb) })}
        </span>
      </div>
      {downloadedModels > 0 && (
        <div className="mt-1 text-xs text-green-600">
          ✓ {t('download.offlineDescription')}
        </div>
      )}
    </div>
  );
