...
export default function AnalyticsConsentSwitch() {
  ...
  const handleCopyUserId = async () => {
    ...
    try {
      ...
      toast.error(t('errors.copyUserIdFailed'));
    }
  };
  ...
  const performToggle = async (enabled: boolean) => {
    ...
    try {
      ...
      // Revert both the UI and the persisted preference. A failure can happen
      // after the store write (for example while analytics is initializing),
      // so rolling back React state alone would leave the next launch in the
      // opposite state from the switch the user sees.
      try {
        const rollbackStore = await load('analytics.json', {
          autoSave: false,
          defaults: {
            analyticsOptedIn: false
          }
        });
        await rollbackStore.set('analyticsOptedIn', !enabled);
        await rollbackStore.set(ANALYTICS_DEFAULT_OFF_MIGRATION_KEY, true);
        await rollbackStore.save();
      } catch (rollbackError) {
        console.error('Failed to restore analytics preference:', rollbackError);
      }
      ...
      if (enabled) {
        try {
          ...
          console.error('Failed to disable analytics after rollback:', rollbackError);
        }
      }

      setIsAnalyticsOptedIn(!enabled);
      toast.error(t('errors.updatePreferenceFailed'));
    } finally {
  ...
  const handlePrivacyPolicyClick = async () => {
    try {
      ...
      toast.error(t('errors.openPrivacyPolicyFailed'));
    }
  };
  ...
  return (
    <>
      <div className="space-y-4">
        <div>
          <h3 className="text-base font-semibold text-gray-800 mb-2">{t('labels.usageAnalytics')}</h3>
          <p className="text-sm text-gray-600 mb-4">
            {t('descriptions.usageAnalyticsIsOffByDefaultYouCanTurnIt')}
          </p>
        </div>

        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
          <div>
            <h4 className="font-semibold text-gray-800">{t('actions.enableAnalytics')}</h4>
            <p className="text-sm text-gray-600">
              {isProcessing ? t('status.updating') : t('labels.offUnlessYouChooseToEnableIt')}
            </p>
          </div>
          <div className="flex items-center gap-2 ml-4">
            ...
            <Switch
              ...
              aria-label={t('accessibility.toggleAnalytics')}
            />
          </div>
        </div>
          ...
          <div className="p-4 border rounded-lg bg-gray-50">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="font-medium text-gray-800 mb-1">{t('labels.yourUserID')}</div>
                <p className="text-xs text-gray-600 mb-2">
                  {t('labels.shareThisIDWhenReportingIssuesToHelpUsInvestigate')}
                </p>
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    title={t('actions.copyUserID')}
                  >
                    {isCopied ? (
                      <>
                        <Check className="w-3.5 h-3.5 text-green-600" />
                        <span className="text-green-600">{t('labels.copied')}</span>
                      </>
                    ) : (
                      <>
        ...
        <div className="flex items-start gap-2 p-2 bg-blue-50 rounded border border-blue-200">
          ...
          <div className="text-xs text-blue-700">
            <p className="mb-1">
              {t('labels.yourMeetingsTranscriptsAndRecordingsRemainCompletelyPrivateAndLocal')}
            </p>
            <button
              onClick={handlePrivacyPolicyClick}
              className="text-blue-600 hover:text-blue-800 underline hover:no-underline"
            >
              {t('actions.viewPrivacyPolicy')}
            </button>
          </div>
        </div>
      </div>
