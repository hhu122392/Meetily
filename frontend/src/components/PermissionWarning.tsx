        ...
        <Alert variant="destructive" className="border-amber-400 bg-amber-50">
          ...
          <AlertTitle className="text-amber-900 font-semibold">
            <div className="flex items-center gap-2">
              ...
              {!hasMicrophone && !hasSystemAudio
                ? t('labels.permissionsRequired')
                : !hasMicrophone
                  ? t('labels.microphonePermissionRequired')
                  : t('labels.systemAudioPermissionRequired')}
            </div>
          </AlertTitle>
          {/* Action Buttons */}
          <div className="mt-4 flex flex-wrap gap-2">
            {isMacOS && !hasMicrophone && (
              ...
              >
                ...
                {t('actions.openMicrophoneSettings')}
              </button>
            )}
            {isMacOS && !hasSystemAudio && (
              ...
              >
                ...
                {t('actions.openScreenRecordingSettings')}
              </button>
            )}
            <button
              onClick={onRecheck}
          ...
          <AlertDescription className="text-amber-800 mt-2">
            ...
            {!hasMicrophone && (
              <>
                <p className="mb-3">
                  {t('descriptions.meetilyNeedsMicrophoneNoDevices')}
                </p>
                <div className="space-y-2 text-sm mb-4">
                  <p className="font-medium">{t('labels.pleaseCheck')}</p>
                  <ul className="list-disc list-inside ml-2 space-y-1">
                    <li>{t('descriptions.microphoneConnected')}</li>
                    <li>{t('descriptions.microphonePermissionGranted')}</li>
                    <li>{t('descriptions.microphoneNotExclusive')}</li>
                  </ul>
                </div>
              </>
            )}
            ...
            {!hasSystemAudio && (
              <>
                <p className="mb-3">
                  {hasMicrophone
                    ? t('descriptions.systemAudioUnavailableMicrophoneWorks')
                    : t('descriptions.systemAudioAlsoUnavailable')}
                </p>
                {isMacOS && (
                  <div className="space-y-2 text-sm mb-4">
                    <p className="font-medium">{t('labels.toEnableSystemAudioOnMacos')}</p>
                    <ul className="list-disc list-inside ml-2 space-y-1">
                      <li>{t('descriptions.installVirtualAudioDevice')}</li>
                      <li>{t('descriptions.grantScreenRecordingPermission')}</li>
                      <li>{t('descriptions.configureAudioRouting')}</li>
                    </ul>
                  </div>
                )}
              </>
