...
export const BlockNoteSummaryView = forwardRef<BlockNoteSummaryViewRef, BlockNoteSummaryViewProps>(({
  ...
  // blocknote 格式下 data.summary_json 是数组，父组件重建对象时引用会变；
  // 记一份指纹，只有内容真的换了才重新屏蔽 onChange
  const loadedSummaryJsonFingerprint = useRef('');
  const savedContentFingerprint = useRef('');
  ...
  useEffect(() => {
    if (format === 'markdown' && data?.markdown && editor) {
      const loadMarkdown = async () => {
        // 这次写入是程序干的（首次加载 / 保存后父组件回写 / 重新生成后刷新），
        // 期间要让 onChange 闭嘴；否则正文会被误标成"有未保存改动"，保存按钮就永远收不回去
        ...
        try {
          ...
          savedContentFingerprint.current = summaryBlocksFingerprint(editor.document);
          latestContentFingerprint.current = savedContentFingerprint.current;
          setIsDirty(false);
  ...
  useEffect(() => {
    if (format === 'blocknote' && data?.summary_json) {
      const fingerprint = JSON.stringify(data.summary_json);
      if (loadedSummaryJsonFingerprint.current === fingerprint) return;
      loadedSummaryJsonFingerprint.current = fingerprint;
      // 同上：initialContent 由父组件回写时也会触发一次 onChange
      isContentLoaded.current = false;
      savedContentFingerprint.current = summaryBlocksFingerprint(data.summary_json);
      latestContentFingerprint.current = savedContentFingerprint.current;
      setIsDirty(false);
  ...
  const handleEditorChange = useCallback((blocks: Block[]) => {
    ...
    if (isContentLoaded.current) {
      ...
      latestContentFingerprint.current = summaryBlocksFingerprint(blocks);
      setIsDirty(latestContentFingerprint.current !== savedContentFingerprint.current);
    }
  }, []);
  ...
  const handleSave = useCallback(async () => {
    ...
    setIsSaving(true);
    const savingFingerprint = summaryBlocksFingerprint(currentBlocks);
    try {
      ...
      savedContentFingerprint.current = savingFingerprint;
      setIsDirty(latestContentFingerprint.current !== savingFingerprint);
      console.log('✅ Save successful');
    } catch (err) {
      console.error('❌ Save failed:', err);
      toast.error(t('errors.failedToSaveChangesPleaseTryAgain'));
      throw err;
    } finally {
      setIsSaving(false);
    }
