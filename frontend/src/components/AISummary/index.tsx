...
interface Props {
  ...
  status: 'idle' | 'processing' | 'summarizing' | 'regenerating' | 'completed' | 'needs_review' | 'error';
  error: string | null;
...
export const AISummary = ({ summary, status, error, onSummaryChange, onRegenerateSummary, meeting }: Props) => {
  ...
  const currentSummary = useMemo(() => {
    if (!summary) {
      return {
        Agenda: { title: t('labels.agenda'), blocks: [] },
        Decisions: { title: t('labels.decisions'), blocks: [] },
        ActionItems: { title: t('labels.actionItems'), blocks: [] },
        ClosingRemarks: { title: t('accessibility.closingRemarks'), blocks: [] }
      };
    }
  ...
  const handleAddSection = () => {
    ...
    const newSummary: Summary = {
      ...currentSummary,
      [newSectionKey]: {
        title: t('accessibility.newSection'),
        blocks: [{
          id: newBlockId,
          type: 'text' as const,
          content: '',
  ...
  const convertToMarkdown = () => {
    let markdown = `# ${t('accessibility.generateAISummary')}: ${meeting?.id || t('labels.unknownMeeting')} - ${meeting?.title || t('labels.untitledMeeting')}\n\n`;
  ...
  const renderErrorState = () => (
    <div className="w-full p-4 bg-red-50 border border-red-200 rounded-lg">
      <div className="flex items-center mb-2">
        ...
        <h3 className="text-red-700 font-medium">{t('errors.errorGeneratingSummary')}</h3>
      </div>
      <p className="text-red-600 text-sm">{t('errors.genericDetails')}</p>
      <p className="text-red-500 text-xs mt-2">{t('labels.pleaseCheckYourModelConfigurationAndAPIKeysOrTry')}</p>
    </div>
  );
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h3 className="text-blue-700 font-medium">
            {status === 'processing' ? t('status.processingTranscript') : t('labels.generatingSummary')}
          </h3>
          <p className="text-blue-600 text-sm">
            {status === 'processing' 
              ? t('status.analyzingYourTranscript')
              : t('status.creatingADetailedSummaryOfYourMeeting')}
          </p>
        </div>
      </div>
    </div>
  ...
  if (!hasContent && (status === 'completed' || status === 'needs_review')) {
    return (
      <div className="w-full p-4 bg-gray-50 border border-gray-200 rounded-lg text-center">
        <p className="text-gray-600">{t('labels.noSummaryContentAvailable')}</p>
        <p className="text-gray-500 text-sm mt-1">{t('actions.tryGeneratingANewSummary')}</p>
      </div>
    );
  }
      ...
      {contextMenu.visible && selectedBlocks.length > 0 && (
        ...
        >
          ...
          >
            ...
            <span>{t('actions.copy')} {t('labels.selectedBlocks', { count: selectedBlocks.length })}</span>
          </button>
          <button
          ...
          >
            <span>🗑️</span>
            <span>{t('actions.delete')} {t('labels.selectedBlocks', { count: selectedBlocks.length })}</span>
          </button>
        </div>
      )}
