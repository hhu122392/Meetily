...
export function PermissionRow({ icon, title, description, status, isPending = false, onAction }: PermissionRowProps) {
  ...
  const getButtonText = () => {
    if (isChecking) return t('actions.checking');
    if (isDenied) return t('actions.openSettings');
    return t('actions.enable');
  };
          ...
          <div className="text-sm text-muted-foreground">
            {isAuthorized ? (
              <span className="text-green-600 flex items-center gap-1">
                ...
                {t('labels.accessGranted')}
              </span>
            ) : isDenied ? (
              <span className="text-red-500 flex items-center gap-1">
                <XCircle className="w-3.5 h-3.5" />
                {t('labels.accessDeniedPleaseGrantInSystemSettings')}
              </span>
            ) : (
