...
export function PermissionsStep() {
  ...
  const handleMicrophoneAction = async () => {
    if (permissions.microphone === 'denied') {
      ...
      try {
        ...
        alert(t('descriptions.pleaseEnableMicrophoneAccessInSystemPreferencesSecurityAndPrivacy'));
      }
      return;
    }
  ...
  const handleSystemAudioAction = async () => {
    if (permissions.systemAudio === 'denied') {
      ...
      try {
        ...
        alert(t('descriptions.pleaseEnableAudioCaptureInSystemSettingsPrivacyAndSecurity'));
      }
      return;
    }
  ...
  return (
    <OnboardingContainer
      title={t('actions.grantPermissions')}
      description={t('descriptions.meetilyNeedsAccessToYourMicrophoneAndSystemAudioTo')}
      step={4}
      hideProgress={true}
      ...
      <div className="max-w-lg mx-auto space-y-6">
        ...
        <div className="space-y-4">
          ...
          <PermissionRow
            ...
            title={t('labels.microphone')}
            description={t('descriptions.requiredToCaptureYourVoiceDuringMeetings')}
          ...
          <PermissionRow
            ...
            title={t('labels.systemAudio')}
            description={t('descriptions.clickEnableToGrantAudioCapturePermission')}
        ...
        <div className="flex flex-col gap-3 pt-4">
          ...
          >
            {t('actions.iLlDoThisLater')}
          </button>

          {!allPermissionsGranted && (
            <p className="text-xs text-center text-muted-foreground">
              {t('status.recordingWonTWorkWithoutPermissionsYouCanGrantThem')}
            </p>
          )}
        </div>
      </div>
