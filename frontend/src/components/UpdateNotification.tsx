...
export function showUpdateNotification(updateInfo: UpdateInfo, onUpdateClick?: () => void) {
  const title = i18n.t('updates:actions.updateAvailable');
  const description = i18n.t('updates:messages.versionIsNowAvailable', { version: updateInfo.version });
  const detailsLabel = i18n.t('updates:labels.viewDetails');
  const handleClick = () => {
    if (onUpdateClick) {
      onUpdateClick();
