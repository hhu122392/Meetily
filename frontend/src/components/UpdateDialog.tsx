...
type UpdateErrorCode = 'updateNoLongerAvailable' | 'prepareFailed' | 'updateNotAvailable' | 'downloadInstallFailed';
...
export function UpdateDialog({ open, onOpenChange, updateInfo }: UpdateDialogProps) {
  ...
  const byteUnits = [t('units.bytes'), t('units.kb'), t('units.mb'), t('units.gb')] as const;
  const [isDownloading, setIsDownloading] = useState(false);
  const [progress, setProgress] = useState<UpdateProgress | null>(null);
  const [error, setError] = useState<UpdateErrorCode | null>(null);
    ...
    if (open && updateInfo?.available) {
      ...
      check().then((updateResult) => {
        if (updateResult?.available) {
          ...
          setError('updateNoLongerAvailable');
        }
      }).catch((err) => {
  ...
  const handleDownloadAndInstall = async () => {
    ...
    if (!updateToUse) {
      try {
        ...
        if (updateResult?.available) {
          ...
          setError('updateNotAvailable');
          return;
        }
      } catch (err: any) {
        console.error('Failed to get update:', err);
        setError('prepareFailed');
        return;
      }
    }
    ...
    try {
      ...
      toast.success(t('messages.updateInstalledSuccessfullyTheAppWillRestart'));
      ...
      setError('downloadInstallFailed');
      setIsDownloading(false);
      toast.error(t('errors.downloadInstallFailed'));
    }
  };
  ...
  const formatDate = (dateString?: string) => {
    ...
    try {
      return new Intl.DateTimeFormat(i18n.resolvedLanguage || i18n.language, { dateStyle: 'medium' }).format(new Date(dateString));
    } catch {
      return dateString;
    }
  };
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            {isDownloading
              ? t('status.downloadingTheLatestVersion')
              : error
              ? t('labels.anErrorOccurredWhileUpdating')
              : t('messages.newVersionAvailable', { version: updateInfo.version })}
          </DialogDescription>
        </DialogHeader>
            ...
            <>
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">{t('labels.currentVersion')}</span>
                  <span className="font-medium">{updateInfo.currentVersion}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">{t('labels.newVersion')}</span>
                  <span className="font-medium text-blue-600">{updateInfo.version}</span>
                </div>
                {updateInfo.date && (
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">{t('labels.releaseDate')}</span>
            ...
            <div className="space-y-2">
              <div className="relative">
                ...
                <div className="flex justify-between text-xs text-gray-600 mt-1">
                  <span>{Math.round(progress.percentage)}{t('labels.complete')}</span>
                  {progress.total > 0 && (
                    <span>
                      {formatBytes(progress.downloaded, i18n.resolvedLanguage || i18n.language, byteUnits)} / {formatBytes(progress.total, i18n.resolvedLanguage || i18n.language, byteUnits)}
                    </span>
                  )}
                </div>
              </div>
              <p className="text-sm text-muted-foreground text-center">
                {t('labels.theAppWillRestartAutomaticallyAfterInstallation')}
              </p>
            </div>
          )}
...
function formatBytes(bytes: number, locale: string, units: readonly string[]): string {
