...
type DatabaseErrorCode = 'noDatabaseFoundAtSelectedLocationPleaseSelectTheMeetily' | 'databaseBrowseFailed' | 'databaseImportFailed' | 'databaseInitializationFailed';

export function LegacyDatabaseImport({ isOpen, onComplete }: LegacyDatabaseImportProps) {
  const { t } = useTranslation('import');
  const [importState, setImportState] = useState<ImportState>('idle');
  const [detectedPath, setDetectedPath] = useState<string | null>(null);
  const [errorCode, setErrorCode] = useState<DatabaseErrorCode | null>(null);
  ...
  const handleBrowse = async () => {
    try {
      ...
      if (dbPath) {
        ...
        setErrorCode('noDatabaseFoundAtSelectedLocationPleaseSelectTheMeetily');
      ...
      setErrorCode('databaseBrowseFailed');
      setImportState('error');
      setTimeout(() => setImportState('idle'), 3000);
    }
  };
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      toast.success(t('messages.databaseImportedSuccessfullyReloading'));
      ...
      setErrorCode('databaseImportFailed');
      setImportState('error');
      toast.error(t('errors.databaseImportFailed'));
      setTimeout(() => setImportState('idle'), 3000);
    }
  };
  ...
  const handleStartFresh = async () => {
    try {
      ...
      setImportState('success');
      toast.success(t('messages.databaseInitializedSuccessfullyStartingApp'));
      ...
      setErrorCode('databaseInitializationFailed');
      setImportState('error');
      toast.error(t('errors.databaseInitializationFailed'));
      setTimeout(() => setImportState('idle'), 3000);
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-[600px]" onPointerDownOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          <DialogTitle className="text-2xl">{t('labels.welcomeToMeetily')}</DialogTitle>
          <DialogDescription className="text-base pt-2">
            {t('prompts.doYouHaveDataFromAPreviousMeetilyInstallation')}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            <p className="text-sm text-gray-600">
              {t('actions.selectYourPreviousMeetilyFolderBackendDirectoryOrDatabaseFile')}
            </p>
            ...
            >
              {importState === 'selecting' || importState === 'detecting' ? (
                <>
                  ...
                  <span>{importState === 'selecting' ? t('status.selecting') : t('status.detectingDatabase')}</span>
                </>
              ) : (
                <>
                  <FolderOpen className="h-5 w-5" />
                  <span>{t('actions.browseForDatabase')}</span>
                </>
              )}
            </button>
          </div>
            ...
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-green-800">{t('labels.databaseFound')}</p>
          ...
          {/* Error Message */}
          {importState === 'error' && errorCode && (
            <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
              <div className="flex items-start gap-2">
                <XCircle className="h-5 w-5 text-red-600 mt-0.5 flex-shrink-0" />
                <div className="flex-1">
                  <p className="text-sm text-red-800">{t(`errors.${errorCode}`)}</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col gap-3 pt-2">
            ...
            >
              {importState === 'importing' ? (
                ...
                <>
                  ...
                  <span>{t('actions.importDatabase')}</span>
                </>
              )}
            </button>
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-white text-gray-500">{t('labels.or')}</span>
              </div>
            </div>
