...
export function HomebrewDatabaseDetector({ onImportSuccess, onDecline }: HomebrewDatabaseDetectorProps) {
  ...
  const formatFileSize = (bytes: number): string => {
    const formatter = new Intl.NumberFormat(i18n.resolvedLanguage === 'zh-CN' ? 'zh-CN' : 'en', {
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="text-sm font-semibold text-blue-900">
              {t('labels.previousMeetilyInstallationDetected')}
            </h3>
          </div>
          <p className="text-sm text-blue-800 mb-2">
            {t('labels.weFoundAnExistingDatabaseFromYourPreviousMeetilyInstallation')}
          </p>
          <div className="bg-white/50 rounded p-2 mb-3">
            ...
            <p className="text-xs text-blue-600 mt-1">
              {t('labels.sizeValue', { size: formatFileSize(dbSize) })}
            </p>
          </div>
          <p className="text-sm text-blue-800 mb-3">
            {t('prompts.wouldYouLikeToImportYourPreviousMeetingsTranscriptsAnd')}
          </p>
