...
import { TranscriptUpdate, SavedTranscript } from '@/types';
...
export class TranscriptService {
   ...
   */
  async getTranscriptHistory(folderPath?: string): Promise<SavedTranscript[]> {
    return invoke<SavedTranscript[]>('get_transcript_history', { folderPath });
  }
  ...
  async onTranscriptUpdate(callback: (update: TranscriptUpdate) => void): Promise<UnlistenFn> {
    return this.onTranscriptUpdates(updates => updates.forEach(callback));
  }

  async onTranscriptUpdates(callback: (updates: TranscriptUpdate[]) => void): Promise<UnlistenFn> {
    const single = await listen<TranscriptUpdate>('transcript-update', event => callback([event.payload]));
    try {
      const batch = await listen<TranscriptUpdate[]>('transcript-update-batch', event => callback(event.payload));
      return () => { single(); batch(); };
    } catch (error) {
      single();
      throw error;
