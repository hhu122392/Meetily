...
/// Tauri embeds Common Controls v6 in the production application manifest, but
/// a Rust test executable does not inherit that activation context. The explicit
/// `app_lib_tests` target lets Cargo scope these linker arguments to tests only,
/// avoiding duplicate manifests in the production binary.
