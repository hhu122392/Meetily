...
pub async fn api_get_meetings<R: Runtime>(
    ...
    let meetings: Result<Vec<MeetingListRow>, sqlx::Error> =
        MeetingsRepository::get_meetings_with_stats(pool).await;
...
/// Update one finalized transcript segment without changing its audio timing.
///
/// The database row, summary source text and the meeting folder's
/// `transcripts.json` mirror are updated as one guarded operation. The file is
/// written while the SQL transaction is still open, so a file-write failure
/// rolls the database change back instead of leaving two different transcripts.
#[tauri::command]
pub async fn api_update_transcript_segment<R: Runtime>(
    ...
    if meeting_id.trim().is_empty() || transcript_id.trim().is_empty() {
        return Err("Meeting ID and transcript ID are required".to_string());
    }
    if text.is_empty() {
    ...
    log_info!(
        "api_update_transcript_segment called for meeting_id: {}, transcript_id: {}",
        meeting_id,
        transcript_id
    );
    ...
    let mut transaction = pool
        .begin()
        .await
        .map_err(|error| format!("Failed to start transcript update: {error}"))?;
    ...
    let Some((folder_path,)) = meeting else {
    ...
    let update =
        sqlx::query("UPDATE transcripts SET transcript = ? WHERE id = ? AND meeting_id = ?")
            ...
            .await
            .map_err(|error| format!("Failed to update transcript: {error}"))?;
    if update.rows_affected() != 1 {
        return Err(format!("Transcript not found in meeting: {transcript_id}"));
    }

    let rows = sqlx::query_as::<_, crate::database::models::Transcript>(
        "SELECT * FROM transcripts WHERE meeting_id = ? ORDER BY audio_start_time ASC",
    )
    .bind(&meeting_id)
    .fetch_all(&mut *transaction)
    .await
    .map_err(|error| format!("Failed to reload transcripts: {error}"))?;
    ...
    sqlx::query("UPDATE transcript_chunks SET transcript_text = ? WHERE meeting_id = ?")
        ...
        .execute(&mut *transaction)
        .await
        .map_err(|error| format!("Failed to update summary source transcript: {error}"))?;
    ...
    if let Some(folder_path) = folder_path.filter(|path| !path.trim().is_empty()) {
        let segments = rows
            ...
            .collect::<Vec<_>>();
        crate::audio::common::write_transcripts_json(std::path::Path::new(&folder_path), &segments)
            .map_err(|error| format!("Failed to update transcripts.json: {error}"))?;
    }

    transaction
        .commit()
        .await
        .map_err(|error| format!("Failed to commit transcript update: {error}"))?;

    let updated = rows
        .into_iter()
        .find(|row| row.id == transcript_id)
        .ok_or_else(|| "Updated transcript could not be reloaded".to_string())?;
    log_info!(
        "Successfully updated transcript {} for meeting {}",
        transcript_id,
        meeting_id
    );
    Ok(MeetingTranscript {
        ...
        audio_start_time: updated.audio_start_time,
        audio_end_time: updated.audio_end_time,
