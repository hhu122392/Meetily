...
/// ParakeetEngine appends its own `parakeet` directory to this root.
pub fn set_models_directory(models_dir: PathBuf) -> Result<(), String> {
    std::fs::create_dir_all(&models_dir)
        .map_err(|error| format!("Failed to create models root for Parakeet: {error}"))?;

    log::info!("Parakeet models directory set to: {}", models_dir.display());

    let mut guard = MODELS_DIR
        .lock()
        .map_err(|_| "Parakeet models directory lock is poisoned".to_string())?;
...
pub async fn parakeet_init() -> Result<(), String> {
    ...
    let models_root = get_models_directory()
        .ok_or_else(|| "Parakeet models directory not initialized".to_string())?;
    let engine = ParakeetEngine::new_with_models_root(models_root)
...
pub async fn parakeet_load_model<R: Runtime>(
    ...
    let _storage_operation_guard = begin_storage_operation(StorageOperationKind::ModelLoad)
