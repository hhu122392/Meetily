    ...
    fn default() -> Self {
        Self {
            ...
            model_status: ModelStatus {
                sensevoice: "not_downloaded".to_string(),
...
pub async fn complete_onboarding<R: Runtime>(
    ...
    // Verify the required model before saving the new onboarding choice.
    crate::sensevoice_engine::commands::sensevoice_validate_model_ready().await?;
    ...
    info!(
        "Saved transcription model config: provider=sensevoice, model={}",
        crate::config::DEFAULT_SENSEVOICE_MODEL
    );
    ...
    status.model_status.sensevoice = "downloaded".to_string();
    // Summary can continue downloading after onboarding; retain its actual status.
