...
impl SettingsRepository {
    ...
    pub async fn get_api_key(
        ...
        let api_key_column = match provider {
            ...
            // Local engines need no API key. Treating an unknown local provider
            // as an error made the whole config read fail, which is how a
            // SenseVoice selection silently fell back to Parakeet (MAIN-045).
            "sensevoice" => return Ok(None),
            _ => {
                return Err(sqlx::Error::Protocol(
                    format!("Invalid provider: {}", provider).into(),
                ))
    ...
    pub async fn save_transcript_api_key(
        ...
        let api_key_column = match provider {
            ...
            "sensevoice" => return Ok(()), // Local sherpa-onnx model, no API key
    ...
    pub async fn get_transcript_api_key(
        ...
        let api_key_column = match provider {
            ...
            "sensevoice" => return Ok(None), // Local sherpa-onnx model, no API key
            "deepgram" => "deepgramApiKey",
            "elevenLabs" => "elevenLabsApiKey",
            "groq" => "groqApiKey",
            "openai" => "openaiApiKey",
