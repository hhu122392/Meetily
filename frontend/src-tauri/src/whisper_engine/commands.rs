...
/// Set the exact Whisper models directory resolved by StorageLayout.
/// This must run during app setup before whisper_init.
pub fn set_models_directory(models_dir: PathBuf) -> Result<(), String> {
    std::fs::create_dir_all(&models_dir)
        .map_err(|error| format!("Failed to create Whisper models directory: {error}"))?;

    log::info!("Models directory set to: {}", models_dir.display());

    let mut guard = MODELS_DIR
        .lock()
        .map_err(|_| "Whisper models directory lock is poisoned".to_string())?;
