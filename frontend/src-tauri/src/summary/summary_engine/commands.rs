...
pub async fn init_model_manager<R: Runtime>(app: &AppHandle<R>) -> anyhow::Result<()> {
    let storage = app.state::<crate::storage::StorageLayoutState>();
    let models_dir = storage.layout().summary_models_dir().to_path_buf();
...
pub async fn builtin_ai_download_model<R: Runtime>(
    ...
    let _storage_operation_guard = begin_storage_operation(StorageOperationKind::ModelDownload)
...
pub async fn builtin_ai_delete_model(
    ...
    let _storage_operation_guard = begin_storage_operation(StorageOperationKind::ModelMutation)
        .map_err(|error| error.to_string())?;
    let manager = {
        let manager_lock = state.0.lock().await;
        manager_lock
...
pub async fn init_model_manager_at_startup<R: Runtime>(app: &AppHandle<R>) -> Result<(), String> {
    init_model_manager(app)
        .await
        .map_err(|error| format!("Failed to initialize ModelManager: {error}"))
}

/// Get the new-install recommendation from effective system RAM.
/// Any platform with less than 14 GiB → qwen3.5:2b.
/// Any platform with at least 14 GiB → qwen3.5:4b.
/// A caller's already-saved valid selection remains authoritative; this
/// command only supplies the recommendation when no selection exists.
#[tauri::command]
...
mod tests {
    ...
    #[test]
    fn recommended_summary_model_uses_qwen2b_below_14_gib_threshold() {
        assert_eq!(recommend_summary_model(true, 13), "qwen3.5:2b");
        assert_eq!(recommend_summary_model(false, 13), "qwen3.5:2b");
    }

    #[test]
    fn recommended_summary_model_uses_qwen4b_at_14_gib_threshold() {
