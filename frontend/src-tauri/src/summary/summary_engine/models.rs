...
/// Resolve model name to a full file path in the exact summary models directory.
pub fn get_model_path(summary_models_dir: &PathBuf, model_name: &str) -> Result<PathBuf> {
    let model =
        get_model_by_name(model_name).ok_or_else(|| anyhow!("Unknown model: {}", model_name))?;

    let model_path = summary_models_dir.join(&model.gguf_file);
