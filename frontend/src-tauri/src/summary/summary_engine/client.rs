...
pub async fn init_sidecar_manager(summary_models_dir: PathBuf) -> Result<()> {
    let manager = SidecarManager::new(summary_models_dir)?;
...
fn get_cached_model_path(summary_models_dir: &PathBuf, model_name: &str) -> Result<PathBuf> {
    let cache_key = format!("{}\u{0}{}", summary_models_dir.display(), model_name);
    ...
    let model_path = models::get_model_path(summary_models_dir, model_name)?;
    ...
    cache.insert(cache_key, model_path.clone());
    Ok(model_path)
}
...
///
/// # Arguments
/// * `summary_models_dir` - Exact built-in summary models directory from StorageLayout
...
pub async fn generate_with_builtin(
    ...
    let model_path = get_cached_model_path(summary_models_dir, model_name)?;
    ...
    let manager = {
        ...
        if global_manager.is_none() {
            ...
            let new_manager = SidecarManager::new(summary_models_dir.clone())?;
            *global_manager = Some(Arc::new(new_manager));
        }
        global_manager.clone().unwrap()
    };

    // A complete built-in generation owns one helper lifecycle. This prevents
    // two simultaneous summaries from sharing a process that the first caller
    // closes immediately after its response.
    let _generation_guard = manager.lock_generation().await;

    // Ensure sidecar is running with this model. A partial startup failure still
    // goes through the same bounded cleanup before the error is returned.
    if let Err(error) = manager.ensure_running(model_path.clone()).await {
        if let Err(cleanup_error) = manager.shutdown().await {
            return Err(error.context(format!(
                "llama-helper cleanup also failed: {}",
                cleanup_error
            )));
        }
        return Err(error);
    ...
    if let Some(token) = cancellation_token {
        if token.is_cancelled() {
            manager
                .shutdown()
                .await
                .context("Failed to close llama-helper after startup cancellation")?;
    ...
    // Every completed, failed, timed-out, or cancelled request closes the helper
    // before control returns to response parsing.
    let response_json =
        request_with_cleanup(&manager, request_json, timeout, cancellation_token).await?;
    ...
    parse_generation_response(&response_json)
}

pub(super) fn parse_generation_response(response_json: &str) -> Result<String> {
...
pub(super) async fn request_with_cleanup(
    manager: &SidecarManager,
    request_json: String,
    timeout: Duration,
    cancellation_token: Option<&CancellationToken>,
) -> Result<String> {
    let request_result = if let Some(token) = cancellation_token {
        tokio::select! {
            result = manager.send_request(request_json, timeout) => result,
            _ = token.cancelled() => {
                log::warn!("Generation cancelled by user");
                Err(anyhow!("Generation cancelled by user"))
            }
        }
    } else {
    ...
    let cleanup_result = manager.shutdown().await;
    match (request_result, cleanup_result) {
        (Ok(response), Ok(())) => Ok(response),
        (Err(error), Ok(())) => Err(error),
        (Ok(_), Err(cleanup_error)) => {
            Err(cleanup_error).context("Generation finished but llama-helper cleanup failed")
        }
...
/// Detaches the current manager and closes it before returning.
