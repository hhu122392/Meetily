...
/// Reads one top-level metadata field while preserving the rest of the document as opaque JSON.
///
/// Template preferences use this instead of a strongly typed meeting metadata struct so recording,
/// import, retranscription and future writers can add fields without being discarded here.
pub(crate) fn read_metadata_field(folder: &Path, field: &str) -> Result<Option<Value>> {
    ...
    let object = value
        .as_object()
        .context("Failed to parse metadata.json: root value must be a JSON object")?;
    Ok(object.get(field).cloned())
}

/// Atomically updates or removes one top-level metadata field under the shared process lock.
pub(crate) fn write_metadata_field(
    folder: &Path,
    field: &str,
...
fn write_language_field_to_metadata(
    ...
    let field_value = match summary_language {
        Some(code) => {
            let normalised = normalise_supported_summary_language(code)?;
            Some(Value::String(normalised))
        }
        None => None,
    };
    write_metadata_field(folder, field, field_value)
}
...
mod tests {
    ...
    #[test]
    fn generic_field_update_preserves_unknown_fields_and_can_remove_value() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(
            dir.path().join("metadata.json"),
            serde_json::to_string_pretty(&json!({
        ...
        write_metadata_field(
            dir.path(),
            "summary_template",
            Some(json!({ "schema_version": 1, "mode": "inherit" })),
        )
        .unwrap();
        assert_eq!(
            read_metadata_field(dir.path(), "summary_template").unwrap(),
            Some(json!({ "schema_version": 1, "mode": "inherit" }))
        );
        ...
        .unwrap();
        assert_eq!(parsed["future_field"]["kept"], true);

        write_metadata_field(dir.path(), "summary_template", None).unwrap();
        assert!(read_metadata_field(dir.path(), "summary_template")
            .unwrap()
            .is_none());
    }
}
