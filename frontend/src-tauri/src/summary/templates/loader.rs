...
use super::v2::parse_and_validate_template_v2;
...
pub fn validate_and_parse_template(json_content: &str) -> Result<Template, String> {
    ...
    if let Some(schema_version) = value.get("schema_version") {
        if schema_version.as_u64() != Some(2) {
            return Err(format!(
        ...
        return parse_and_validate_template_v2(json_content)
            .map(|template| template.to_runtime_template())
            .map_err(|validation| {
                let issue_paths = validation
                    ...
                    .map(|issue| {
                        if issue.path.is_empty() {
                            ...
                            format!("{} at {}", issue.code, issue.path)
                        }
                    })
                    .collect::<Vec<_>>()
                    .join(", ");
                format!("Template v2 validation failed: {issue_paths}")
            });
    }
...
mod tests {
    ...
    #[test]
    fn test_rejects_explicit_unsupported_schema_version() {
        let future_template = serde_json::json!({
            "schema_version": 3,
            "name": "Future template",
            "description": "Must not silently fall back to v1",
            "sections": [{
                "title": "Summary",
        ...
        let result = validate_and_parse_template(&future_template.to_string());
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .contains("Unsupported template schema version"));
    }
}
