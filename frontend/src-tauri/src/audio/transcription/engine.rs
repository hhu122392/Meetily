...
pub enum TranscriptionEngine {
    SenseVoice(Arc<crate::sensevoice_engine::SenseVoiceEngine>),
...
async fn recording_transcript_config<R: Runtime>(app: &AppHandle<R>) -> Result<crate::api::api::TranscriptConfig, String> {
    crate::api::api::api_get_transcript_config(app.clone(), app.clone().state(), None)
        .await
        .map(|config| config.unwrap_or_else(|| crate::api::api::TranscriptConfig {
            provider: "sensevoice".to_owned(),
            model: crate::config::DEFAULT_SENSEVOICE_MODEL.to_owned(),
            api_key: None,
        }))
        .map_err(|error| format!("Failed to read transcription configuration: {error}"))
}

pub async fn uses_sensevoice<R: Runtime>(app: &AppHandle<R>) -> Result<bool, String> {
    Ok(recording_transcript_config(app).await?.provider == "sensevoice")
}

fn validate_recording_language(provider: &str, model: &str, language: &str) -> Result<(), String> {
    let language = crate::normalize_transcription_language(language)?;
    if provider == "parakeet" {
        // 'auto' must cover Chinese too, not silently restrict detection to
        // Parakeet's European languages. Manual hints are unsupported as well.
        return Err(format!("Source language '{language}' requires multilingual Whisper. Prepare a multilingual model before recording."));
    }
    if provider == "localWhisper"
        && !crate::config::WHISPER_MODEL_CATALOG
            .iter()
            .any(|entry| entry.0 == model)
    {
        return Err(format!(
            "Model '{model}' is not a supported multilingual Whisper model."
        ));
    }
    Ok(())
}
...
pub async fn validate_transcription_model_ready<R: Runtime>(
    ...
    validate_recording_language(
        &config.provider,
        &config.model,
        &crate::get_language_preference_internal().ok_or("Source language is unavailable")?,
    )?;
    ...
    match config.provider.as_str() {
        "localWhisper" => {
            ...
            match get_or_init_whisper(app).await {
                Ok(engine) => {
                    let model_name = engine.get_current_model().await;
                    if model_name.as_deref() != Some(config.model.as_str())
        ...
        "sensevoice" => {
            ...
            match crate::sensevoice_engine::commands::sensevoice_validate_model_ready().await {
                Ok(model_name) => {
...
#[cfg(test)]
mod recording_language_tests {
    use super::validate_recording_language;

    #[test]
    fn chinese_and_automatic_detection_cannot_use_parakeet() {
        for language in ["zh", "zh-CN", "auto", "auto-translate", "en"] {
            assert!(
                validate_recording_language("parakeet", "parakeet-tdt-0.6b-v3-int8", language)
                    .is_err()
            );
        }
    }
    ...
    #[test]
    fn multilingual_whisper_preserves_supported_source_intent() {
        for language in ["zh", "zh-CN", "auto", "en"] {
            assert!(validate_recording_language("localWhisper", "small", language).is_ok());
        }
        assert!(validate_recording_language("localWhisper", "small.en", "zh").is_err());
        assert!(validate_recording_language("localWhisper", "unknown", "auto").is_err());
        assert!(validate_recording_language("localWhisper", "small", "").is_err());
    }
}
...
pub async fn get_or_init_transcription_engine<R: Runtime>(
    ...
    match config.provider.as_str() {
        "sensevoice" => {
            info!("🎧 Initializing SenseVoice (sherpa-onnx) transcription engine");

            let engine = crate::sensevoice_engine::commands::get_engine().ok_or_else(|| {
            ...
            if !engine.is_model_loaded().await {
                return Err(
                    "SenseVoice engine is initialized but no model is loaded. This should not happen after validation."
                        .to_string(),
                );
            }
            ...
            info!("✅ SenseVoice model '{}' is loaded", model_name);
