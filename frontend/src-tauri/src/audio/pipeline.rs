...
use super::measurement::D11MeasurementRecorder;
use super::recording_state::{
    AudioChunk, AudioError, DeviceType, InFlightAudioCallback, RecordingState,
};
use super::vad::ContinuousVadProcessor;

/// Upper bound for one live ASR request.
///
/// Upstream issue #756 asks for a 20 second target with a 25 second hard
/// maximum, because VAD alone can leave a live segment open indefinitely.
/// Our previous 3.5/5 second bound was far more aggressive: it produced
/// sub-4 second requests, which upstream classifies as a defect (PR #679)
/// and which measurably costs word accuracy here. On a real 20 second window
/// the same audio transcribed "V4 Pro / V4.1 Flash" correctly as one request,
/// while the same speech decoded from short requests lost those terms.
/// VAD still closes a segment earlier whenever the speaker pauses.
pub(crate) fn live_segment_durations_ms() -> (u32, u32) {
    (20_000, 25_000)
}
...
impl AudioMixerRingBuffer {
    fn new(sample_rate: u32, microphone_enabled: bool, system_enabled: bool) -> Self {
        let window_ms = 600.0;
        let window_size_samples = (sample_rate as f32 * window_ms / 1000.0) as usize;
        let max_buffer_size = window_size_samples * 8;

        info!(
            "🔊 Ring buffer initialized: window={}ms ({} samples), max={}ms ({} samples), microphone={}, system={}",
            window_ms,
            window_size_samples,
            window_ms * 8.0,
            max_buffer_size,
    ...
    fn add_samples(&mut self, device_type: DeviceType, samples: Vec<f32>) {
        ...
        // Safety: cap each route at eight 600ms windows (4.8 seconds).
    ...
    fn can_mix(&self) -> bool {
        (self.microphone_enabled || self.system_enabled)
            && (!self.microphone_enabled || self.mic_buffer.len() >= self.window_size_samples)
    ...
    fn extract_tail_window(&mut self) -> Option<(Vec<f32>, Vec<f32>)> {
        let microphone_samples = self.microphone_enabled.then_some(self.mic_buffer.len());
        let system_samples = self.system_enabled.then_some(self.system_buffer.len());
        let tail_size = microphone_samples
            .into_iter()
            .chain(system_samples)
            .max()
            .unwrap_or(0)
        ...
        Some((
            Self::drain_route(&mut self.mic_buffer, self.microphone_enabled, tail_size),
            Self::drain_route(&mut self.system_buffer, self.system_enabled, tail_size),
        ))
    }

    fn drain_route(buffer: &mut VecDeque<f32>, enabled: bool, window_size: usize) -> Vec<f32> {
        if !enabled {
            return vec![0.0; window_size];
        }
        let available = buffer.len().min(window_size);
        let mut window: Vec<f32> = buffer.drain(0..available).collect();
...
impl AudioCapture {
    pub fn new(
        ...
        let device_epoch = state.get_device_epoch();
        ...
        let (noise_suppressor, high_pass_filter, normalizer) = if matches!(
            ...
            // Initialize the bounded EBU R128 normalizer. Its upward gain is
            // capped so microphone idle noise cannot be promoted to speech level.
            let norm = match LoudnessNormalizer::new(1, TARGET_SAMPLE_RATE) {
                Ok(normalizer) => {
                    info!(
                        "✅ Bounded EBU R128 normalizer initialized for microphone '{}' (target: -23 LUFS, max upward gain: +6 dB)",
                        device.name
                    );
                    Some(normalizer)
                }
    ...
    pub fn process_audio_data(&self, data: &[f32]) {
        self.process_audio_data_at_qpc(data, None);
    }

    pub fn process_audio_data_at_qpc(&self, data: &[f32], capture_qpc_ns: Option<u64>) {
        // Check if still recording
        if !self.state.is_recording() {
            return;
        }
        // The guard keeps cutover accounting open until this callback has
        // finished. Old-epoch frames after the QPC watermark are rejected.
        let frame_count = data.len().checked_div(self.channels as usize).unwrap_or(0) as u64;
        let Some(_callback_guard) =
            self.state
                .begin_audio_callback(self.device_epoch, capture_qpc_ns, frame_count)
        else {
            return;
        };
        self.process_audio_data_with_guard(data, capture_qpc_ns, _callback_guard);
    }

    pub(crate) fn process_audio_data_with_guard(
        &self,
        data: &[f32],
        capture_qpc_ns: Option<u64>,
        _callback_guard: InFlightAudioCallback,
    ) {
        #[cfg(target_os = "windows")]
        if self.device_type == DeviceType::Microphone {
            match super::windows_loopback::qpc_now_ns() {
                Ok(observed_qpc_ns) => self
                    .state
                    .record_microphone_callback_qpc(capture_qpc_ns, observed_qpc_ns),
                Err(error) => {
                    error!("Microphone callback QPC read failed: {error:#}");
                    self.state.report_route_stream_error(
                        &DeviceType::Microphone,
        ...
        self.state
            .record_route_callback(&self.device_type, data, self.channels);

        self.process_audio_samples(data, capture_qpc_ns);
    }

    /// Fill an idle Windows render interval without fabricating capture metrics.
    #[cfg(target_os = "windows")]
    pub(crate) fn process_timeline_silence(&self, frames: usize) -> bool {
        if self.device_type != DeviceType::System {
            return false;
        }
        let Some(_guard) = self.state.begin_timeline_padding(self.device_epoch) else {
            return false;
        };
        self.process_audio_samples(&vec![0.0; frames * self.channels as usize], None);
        true
    }

    fn process_audio_samples(&self, data: &[f32], capture_qpc_ns: Option<u64>) {
        ...
        if matches!(self.device_type, DeviceType::System) {
            if let Some(qpc_ns) = capture_qpc_ns {
                self.state.mark_system_stream_started(qpc_ns);
            }
        }
    ...
    pub fn handle_stream_error(&self, error: cpal::StreamError) {
        ...
        self.state
            .report_route_stream_error(&self.device_type, audio_error);
    }
}
...
pub struct AudioPipeline {
    ...
    last_device_epoch: u64,
    last_capture_qpc_ns: Option<u64>,
    last_recording_timestamp: f64,
...
impl AudioPipeline {
    pub fn new(
        ...
        // Create VAD processor with enough redemption time to keep a short
        // Mandarin pause inside the same phrase. The former 400ms Windows value
        // emitted many 1-3 second fragments; those fragments lacked enough
        // acoustic context for reliable live decoding.
        // The VAD processor now handles 48kHz->16kHz resampling internally
        // This bridges natural pauses without excessive fragmentation
        let redemption_time = 1_200;

        let vad_processor = match ContinuousVadProcessor::new(sample_rate, redemption_time) {
            Ok(processor) => {
                // Keep the first result within the eight-second UI target, then
                // use five-second windows so Chinese phrases are not decoded as
                // a series of unrelated two-second fragments.
                let (first_live_segment_ms, subsequent_live_segment_ms) =
                    live_segment_durations_ms();
                info!(
                    "VAD-driven pipeline: first live segment <= {}ms, subsequent segments <= {}ms",
                    first_live_segment_ms, subsequent_live_segment_ms
                );
                processor.with_adaptive_live_segment_duration_ms(
                    first_live_segment_ms,
                    subsequent_live_segment_ms,
                )
            }
            Err(e) => {
        ...
        let ring_buffer =
            AudioMixerRingBuffer::new(sample_rate, microphone_enabled, system_enabled);
        ...
        Self {
            ...
            last_device_epoch: 0,
            last_capture_qpc_ns: None,
            last_recording_timestamp: 0.0,
            measurement_recorder,
        }
    }

    fn process_ready_windows(&mut self, recording_timestamp: f64) {
        while let Some((mic_window, sys_window)) = self.ring_buffer.extract_window() {
            self.process_mixed_window(mic_window, sys_window, recording_timestamp);
        }
    }

    fn flush_mixer_tail(&mut self) {
        while let Some((mic_window, sys_window)) = self.ring_buffer.extract_tail_window() {
            self.process_mixed_window(mic_window, sys_window, self.last_recording_timestamp);
        }
    }
    ...
    fn process_mixed_window(
        ...
        mic_window: Vec<f32>,
        sys_window: Vec<f32>,
        recording_timestamp: f64,
    ) {
        let mixed_with_gain = self.mixer.mix_window(&mic_window, &sys_window);

        let vad_before = self.vad_processor.trace_state();
        let mut emitted_chunk_ids = Vec::new();
        match self.vad_processor.process_audio(&mixed_with_gain) {
                ...
                for segment in speech_segments {
                    ...
                    if segment.samples.len() >= 800 {
                        let source_sample_start =
                            (segment.start_timestamp_ms * 16.0).round() as u64;
                        let source_sample_end = (segment.end_timestamp_ms * 16.0).round() as u64;
                        if let Some(recorder) = self.measurement_recorder.as_ref() {
                            recorder.record_vad_chunk(
                        ...
                        let transcription_chunk = AudioChunk {
                            ...
                            device_epoch: self.last_device_epoch,
                            capture_qpc_ns: self.last_capture_qpc_ns,
                        };
                        ...
                        if let Err(e) = self.transcription_sender.send(transcription_chunk) {
                            ...
                            if let Some(recorder) = self.measurement_recorder.as_ref() {
                                recorder.record_error(format!(
                                    "failed to send VAD chunk {}: {e}",
                                    self.chunk_id_counter
                                ));
                            }
                        } else {
                            emitted_chunk_ids.push(self.chunk_id_counter);
                        ...
                        if let Some(recorder) = self.measurement_recorder.as_ref() {
                            recorder.record_vad_window(
                                (segment.start_timestamp_ms * 16.0).round() as u64,
                                (segment.end_timestamp_ms * 16.0).round() as u64,
                                "excluded",
                                Some("shorter_than_800_samples"),
                                Vec::new(),
                            );
                        }
                    }
            ...
            Err(e) => {
                warn!("⚠️ VAD error: {}", e);
                if let Some(recorder) = self.measurement_recorder.as_ref() {
                    recorder.record_error(format!("VAD processing failed: {e}"));
                }
            }
        }
        let vad_after = self.vad_processor.trace_state();
        if let Some(recorder) = self.measurement_recorder.as_ref() {
            let (decision, exclusion) = if !emitted_chunk_ids.is_empty() {
                ("speech_emitted", None)
            } else if vad_after.in_speech
                || vad_after.buffered_speech_samples > 0
                || vad_after.buffered_input_samples > 0
            {
                ("buffered", Some("awaiting_complete_vad_segment"))
            } else {
                ("excluded", Some("vad_non_speech"))
            };
            recorder.record_vad_window(
                vad_before.processed_samples as u64,
                vad_after.processed_samples as u64,
                decision,
                exclusion,
                emitted_chunk_ids,
            );
        ...
        loop {
            ...
            .await
            {
                Ok(Some(chunk)) => {
                    ...
                    if self.processed_chunks > 0 && self.last_device_epoch != chunk.device_epoch {
                        info!(
                            "Audio device epoch changed {} -> {}; draining the old mixer tail",
                            self.last_device_epoch, chunk.device_epoch
                        );
                        self.flush_mixer_tail();
                    }
                    self.last_device_epoch = chunk.device_epoch;
                    self.last_capture_qpc_ns = chunk.capture_qpc_ns;
                    self.last_recording_timestamp = chunk.timestamp;
                    ...
                    self.process_ready_windows(chunk.timestamp);
                }
                Ok(None) => {
                    info!(
    ...
    fn flush_remaining_audio(&mut self) -> Result<()> {
        ...
        match self.vad_processor.flush() {
                ...
                for segment in final_segments {
                    ...
                    if segment.samples.len() >= 800 {
                        ...
                        if let Err(e) = self.transcription_sender.send(transcription_chunk) {
                            ...
                            if let Some(recorder) = self.measurement_recorder.as_ref() {
                                recorder.record_error(format!(
                                    "failed to send final VAD chunk {}: {e}",
                                    self.chunk_id_counter
                                ));
                            }
                        } else {
            ...
            Err(e) => {
                ...
                if let Some(recorder) = self.measurement_recorder.as_ref() {
                    recorder.record_error(format!("failed to flush VAD processor: {e}"));
                }
            }
        }
...
pub struct AudioPipelineManager {
    ...
    preserve_transcription_gaps: bool,
}
...
impl AudioPipelineManager {
    pub fn new() -> Self {
        Self {
            ...
            audio_sender: None,
            preserve_transcription_gaps: false,
        }
    }

    pub fn set_preserve_transcription_gaps(&mut self, enabled: bool) {
        self.preserve_transcription_gaps = enabled;
    }
    ...
    pub fn start(
        ...
        if self.preserve_transcription_gaps {
            pipeline.vad_processor.enable_gap_preservation();
        }
...
#[cfg(test)]
mod live_latency_configuration_tests {
    use super::live_segment_durations_ms;

    #[test]
    fn live_segments_preserve_first_bubble_and_continuous_update_margin() {
        // Bounded live segments follow upstream issue #756 (20 s target,
        // 25 s hard maximum) instead of the previous 3.5/5 s fragmentation.
        assert_eq!(live_segment_durations_ms(), (20_000, 25_000));
    }
}
