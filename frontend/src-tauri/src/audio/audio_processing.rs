...
const MAX_LIVE_MIC_UPWARD_GAIN_DB: f64 = 6.0;
const RNNOISE_PCM_SCALE: f32 = 32768.0;

fn bounded_live_mic_gain_db(target_lufs: f64, current_lufs: f64) -> f64 {
    (target_lufs - current_lufs).min(MAX_LIVE_MIC_UPWARD_GAIN_DB)
}

fn to_rnnoise_pcm(sample: f32) -> f32 {
    sample.clamp(-1.0, 1.0) * RNNOISE_PCM_SCALE
}

fn from_rnnoise_pcm(sample: f32) -> f32 {
    (sample / RNNOISE_PCM_SCALE).clamp(-1.0, 1.0)
}
...
pub fn create_meeting_folder(
    ...
    // P2-22：目录名用本地时间，和界面/会议标题里的时间保持一致（原来用 UTC，差 8 小时）
    ...
    // Claim a fresh directory atomically. Two recordings with the same title
    // in one minute must never share or overwrite a meeting directory.
    let meeting_folder = (0..10_000)
        .find_map(|index| {
            let folder_name = if index == 0 {
                base_folder_name.clone()
            } else {
                format!("{base_folder_name}-{index}")
            };
        ...
        .transpose()?
        .ok_or_else(|| anyhow::anyhow!("unable to allocate a unique meeting directory"))?;
...
impl LoudnessNormalizer {
    ...
    pub fn normalize_loudness(&mut self, samples: &[f32]) -> Vec<f32> {
        ...
        // Live microphones frequently contain a measurable idle noise floor.
        // EBU R128 treats that stationary noise as programme material and, without
        // a bound, can raise it to the speech target. A modest +6 dB ceiling keeps
        // quiet speech usable without turning room noise into a broadband signal.
        const ANALYZE_CHUNK_SIZE: usize = 512;
                ...
                if let Err(e) = self.ebur128.add_frames_f32(&self.loudness_buffer) {
                    ...
                    if let Ok(current_lufs) = self.ebur128.loudness_global() {
                        if current_lufs.is_finite() && current_lufs < 0.0 {
                            let gain_db = bounded_live_mic_gain_db(TARGET_LUFS, current_lufs);
...
impl NoiseSuppressionProcessor {
    ...
    pub fn process(&mut self, samples: &[f32]) -> Vec<f32> {
        ...
        while self.frame_buffer.len() >= self.frame_size {
            // Extract one frame
            // nnnoiseless follows the RNNoise C API and expects floating-point
            // samples expressed on the 16-bit PCM scale, not CPAL's [-1, 1]
            // normalized scale.
            ...
            output.extend(denoised_frame.into_iter().map(from_rnnoise_pcm));
        }
    ...
    pub fn flush(&mut self) -> Vec<f32> {
        ...
        let mut input_frame: Vec<f32> = self
            .frame_buffer
            .iter()
            .copied()
            .map(to_rnnoise_pcm)
        ...
        for sample in &mut output {
            *sample = from_rnnoise_pcm(*sample);
        }
...
#[cfg(test)]
mod live_microphone_processing_tests {
    use super::*;

    #[test]
    fn rnnoise_scale_round_trip_preserves_normalized_samples() {
        for sample in [-1.0_f32, -0.25, 0.0, 0.25, 0.999] {
            let round_trip = from_rnnoise_pcm(to_rnnoise_pcm(sample));
            assert!((round_trip - sample).abs() < 1.0e-6);
        }
        assert_eq!(to_rnnoise_pcm(2.0), RNNOISE_PCM_SCALE);
        assert_eq!(from_rnnoise_pcm(RNNOISE_PCM_SCALE * 2.0), 1.0);
    }

    #[test]
    fn live_microphone_upward_gain_is_capped_at_six_db() {
        assert_eq!(bounded_live_mic_gain_db(-23.0, -60.0), 6.0);
        assert_eq!(bounded_live_mic_gain_db(-23.0, -26.0), 3.0);
        assert_eq!(bounded_live_mic_gain_db(-23.0, -20.0), -3.0);
    }

    #[test]
    fn rnnoise_processor_returns_finite_normalized_audio() {
        let mut processor = NoiseSuppressionProcessor::new(48_000).unwrap();
        let input: Vec<f32> = (0..(DenoiseState::FRAME_SIZE * 2))
