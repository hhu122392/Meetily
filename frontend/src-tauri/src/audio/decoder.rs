...
#[error("audio exceeds the frozen duration limit")]
pub struct AudioDurationLimitExceeded;
...
impl DecodedAudio {
    ...
    /// Produce one normalized 16 kHz mono stream before MOSS chunking. This
    /// keeps 44.1/48 kHz requests on exact 4,800,000-sample native boundaries
    /// and checks cancellation/deadline work throughout conversion.
    pub fn to_moss_format_with_cancel<F>(&self, should_cancel: F) -> Result<Vec<f32>>
    where
        F: Fn() -> bool,
    {
        if self.channels == 0 || self.sample_rate == 0 {
            return Err(anyhow!("invalid decoded audio format"));
        }
        let channels = self.channels as usize;
        if self.samples.len() % channels != 0 {
            return Err(anyhow!("decoded audio is not frame aligned"));
        }
        let mut mono = Vec::with_capacity(self.samples.len() / channels);
        for frames in self.samples.chunks(channels * 4_096) {
            if should_cancel() {
                return Err(anyhow!("audio normalization cancelled"));
            }
        ...
        for samples in mono.chunks(4_096) {
            if should_cancel() {
                return Err(anyhow!("audio normalization cancelled"));
            }
        ...
        let scale = if maximum > 1.0 { 1.0 / maximum } else { 1.0 };
        for samples in mono.chunks_mut(4_096) {
            if should_cancel() {
                return Err(anyhow!("audio normalization cancelled"));
            }
            for sample in samples {
        ...
        let chunk_samples = self.sample_rate as usize * 60;
        let overlap_input = self.sample_rate as usize / 10;
        let ratio = 16_000.0 / self.sample_rate as f64;
        let overlap_output = (overlap_input as f64 * ratio) as usize;
        let mut output = Vec::with_capacity((mono.len() as f64 * ratio) as usize + 1_024);
        let mut start = 0usize;
        while start < mono.len() {
            if should_cancel() {
                return Err(anyhow!("audio normalization cancelled"));
            }
            let end = (start + chunk_samples + overlap_input).min(mono.len());
            let mut resampled = resample(&mono[start..end], self.sample_rate, 16_000)?;
            fit_resampled_length(
                &mut resampled,
                exact_resampled_length(end - start, self.sample_rate, 16_000)?,
            );
            if start == 0 {
                ...
                for index in 0..fade_len {
                    let factor = index as f32 / fade_len.max(1) as f32;
                    output[output_start + index] =
                        output[output_start + index] * (1.0 - factor) + resampled[index] * factor;
                }
                output.extend_from_slice(&resampled[fade_len..]);
            }
            start = start.saturating_add(chunk_samples);
        }
        fit_resampled_length(
            &mut output,
            exact_resampled_length(mono.len(), self.sample_rate, 16_000)?,
        );
        for samples in output.chunks_mut(4_096) {
            if should_cancel() {
                return Err(anyhow!("audio normalization cancelled"));
            }
            for sample in samples {
...
fn exact_resampled_length(input_samples: usize, from_rate: u32, to_rate: u32) -> Result<usize> {
    if from_rate == 0 || to_rate == 0 {
        return Err(anyhow!("invalid sample rate"));
    }
    let numerator = (input_samples as u128)
        .checked_mul(to_rate as u128)
        .and_then(|value| value.checked_add((from_rate / 2) as u128))
        .ok_or_else(|| anyhow!("resampled audio length overflow"))?;
    usize::try_from(numerator / from_rate as u128)
        .map_err(|_| anyhow!("resampled audio length overflow"))
}

fn fit_resampled_length(samples: &mut Vec<f32>, expected: usize) {
    if samples.len() > expected {
        samples.truncate(expected);
    } else if samples.len() < expected {
        let fill = samples.last().copied().unwrap_or(0.0);
        samples.resize(expected, fill);
    }
}
...
fn convert_to_wav_with_ffmpeg(
    ...
    if should_cancel() {
        return Err(anyhow!("audio decoding cancelled"));
    }
    ...
    if !status.success() {
        error!("FFmpeg conversion failed (exit code: {})", status);
        return Err(anyhow!(
...
/// Decode an audio file while checking a caller-owned cancellation/deadline flag.
/// The callback is checked before native work, while ffmpeg is running, and once
/// per decoded packet so a cancelled MOSS request cannot proceed to staging.
pub fn decode_audio_file_with_cancel<F>(path: &Path, should_cancel: F) -> Result<DecodedAudio>
where
    F: Fn() -> bool,
{
    decode_audio_file_internal(path, None, None, &should_cancel)
}

/// Decode with cancellation and a hard duration ceiling. Metadata is checked
/// before packet decoding when available, and decoded frames are bounded again
/// while reading so malformed or metadata-free inputs cannot bypass the limit.
pub fn decode_audio_file_with_cancel_and_limit<F>(
    path: &Path,
    ...
    F: Fn() -> bool,
{
    if maximum_duration_seconds == 0 {
        return Err(anyhow!("audio duration limit is invalid"));
    }
    decode_audio_file_internal(path, None, Some(maximum_duration_seconds), &should_cancel)
}
...
pub fn decode_audio_file_with_progress(
    ...
    decode_audio_file_internal(path, progress_callback, None, &|| false)
}

fn decode_audio_file_internal(
    path: &Path,
    progress_callback: Option<ProgressCallback>,
    ...
    let (_temp_wav_guard, decode_path): (Option<tempfile::TempPath>, Cow<'_, Path>) =
        if needs_ffmpeg_conversion(path) {
            ...
            let temp_path =
                convert_to_wav_with_ffmpeg(path, progress_callback.as_ref(), should_cancel)?;
    ...
    if maximum_duration_seconds
        .is_some_and(|maximum| expected_duration.is_some_and(|duration| duration > maximum as f64))
    {
        return Err(AudioDurationLimitExceeded.into());
    }
    let maximum_samples = maximum_duration_seconds
        .and_then(|maximum| {
            (sample_rate as u64)
        ...
        match decoder.decode(&packet) {
            Ok(decoded) => {
                ...
                if let Some(ref mut buf) = sample_buf {
                    ...
                    if maximum_samples.is_some_and(|maximum| {
                        all_samples.len().saturating_add(buf.samples().len()) > maximum
                    }) {
                        return Err(AudioDurationLimitExceeded.into());
                    }
...
mod tests {
    ...
    #[test]
    fn moss_normalizes_complete_44_1_and_48_khz_stereo_streams_once() {
        for source_rate in [44_100u32, 48_000u32] {
            let frames = source_rate as usize * 2;
            let mut interleaved = Vec::with_capacity(frames * 2);
            for index in 0..frames {
                let sample = ((index % 997) as f32 / 997.0) * 0.5;
            ...
            let normalized = audio.to_moss_format_with_cancel(|| false).unwrap();
            assert_eq!(normalized.len(), 32_000, "source rate {source_rate}");
    ...
    #[test]
    fn moss_normalization_observes_cancellation_before_resampling() {
        let audio = DecodedAudio {
        ...
        assert!(audio.to_moss_format_with_cancel(|| true).is_err());
    }
