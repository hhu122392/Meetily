...
pub struct AudioDevice {
    ...
    /// Stable platform endpoint identifier. Windows selections must use this
    /// value; `name` is display text only and is not unique.
...
impl AudioDevice {
    ...
    pub fn with_native_id(name: String, device_type: DeviceType, native_id: String) -> Self {
        AudioDevice {
            name,
            device_type,
    ...
    pub fn with_default_role(mut self, role: &str) -> Self {
        self.default_role = Some(role.to_string());
        self
    }

    pub fn selection_key(&self) -> String {
        self.native_id.clone().unwrap_or_else(|| self.to_string())
    }
