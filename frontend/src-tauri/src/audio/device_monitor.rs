...
impl MonitoredDevice {
    fn new(device: &AudioDevice, device_type: DeviceMonitorType) -> Self {
        ...
        let is_bluetooth = name.to_lowercase().contains("airpods")
            ...
            || name.to_lowercase().contains("freeclip")
            || name.to_lowercase().contains("huawei");

        Self {
            name,
            native_id: device.native_id.clone(),
            default_role: device.default_role.clone(),
            device_type,
...
impl AudioDeviceMonitor {
    ...
    pub fn start_monitoring(
        ...
        if let Some(mic) = microphone {
            monitored_devices.push(MonitoredDevice::new(&mic, DeviceMonitorType::Microphone));
            info!(
        ...
        if let Some(sys) = system_audio {
            monitored_devices.push(MonitoredDevice::new(&sys, DeviceMonitorType::SystemAudio));
            info!(
    ...
    async fn monitor_loop(
        ...
        loop {
            ...
            for monitored in &mut monitored_devices {
                ...
                if monitored.default_role.is_some()
                    && monitored.device_type == DeviceMonitorType::SystemAudio
                {
                    if let Ok(default_id) =
                        super::devices::platform::default_windows_endpoint_id(DeviceType::Output)
                    {
                        if monitored.native_id.as_deref() != Some(default_id.as_str()) {
                            if let Some(replacement) = current_devices.iter().find(|device| {
                                device.native_id.as_deref() == Some(default_id.as_str())
                            }) {
                                let old_name = monitored.name.clone();
                                let old_native_id = monitored.native_id.clone();
                                monitored.name = replacement.name.clone();
                                monitored.native_id = replacement.native_id.clone();
                                ...
                                let _ = event_sender.send(DeviceEvent::DeviceReconnected {
                                    ...
                                    native_id: monitored.native_id.clone(),
                                    device_type: monitored.device_type.clone(),
                                });
                                continue;
                            }
                ...
                let device_found = current_devices.iter().any(|device| {
                    if let Some(native_id) = monitored.native_id.as_deref() {
                        device.native_id.as_deref() == Some(native_id)
                    } else {
                        device.name == monitored.name
                    }
                });
...
mod tests {
    ...
    fn test_bluetooth_detection() {
        let airpods_device = AudioDevice::new("John's AirPods Pro".to_string(), DeviceType::Input);
        let airpods = MonitoredDevice::new(&airpods_device, DeviceMonitorType::Microphone);
        assert!(airpods.is_bluetooth);
        assert_eq!(airpods.disconnect_threshold(), 3);

        let builtin_device = AudioDevice::new("Built-in Microphone".to_string(), DeviceType::Input);
        let builtin = MonitoredDevice::new(&builtin_device, DeviceMonitorType::Microphone);
