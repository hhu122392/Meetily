...
pub(super) const AAC_CODEC_ARGS: [&str; 6] =
...
pub fn encode_single_audio(
    ...
    encode_audio(data, sample_rate, channels, output_path, &AAC_CODEC_ARGS)
}
...
pub(super) fn encode_audio_checkpoint(
    ...
    // ALAC retains the sample count; independently padded AAC chunks cannot
    // be concatenated without moving every subsequent checkpoint's clock.
    encode_audio(data, sample_rate, 1, output_path, &["-c:a", "alac"])
}
