...
SCRIPT = Path(__file__).with_name("moss_v3_p0r_dense_window.py")
SPEC = importlib.util.spec_from_file_location("moss_v3_p0r_dense_window", SCRIPT)
...
class DenseWindowTests(unittest.TestCase):
    def test_window_is_continuous_and_policy_sized(self) -> None:
        self.assertEqual(MODULE.SOURCE_END_SECONDS - MODULE.SOURCE_START_SECONDS, 180.0)
        self.assertGreaterEqual(MODULE.WINDOW_DURATION_SECONDS, 180.0)
        self.assertLessEqual(MODULE.WINDOW_DURATION_SECONDS, 300.0)
        self.assertEqual(MODULE.MINIMUM_SPEECH_SECONDS, 153.0)
    ...
    def test_real_candidate_window_exceeds_frozen_speech_gate(self) -> None:
        ...
        self.assertAlmostEqual(speech, 173.651, places=3)
